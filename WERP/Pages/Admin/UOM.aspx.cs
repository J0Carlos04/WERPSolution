...
public partial class Pages_UOM : PageBase
{    
    #region Handlers
    ...
    protected void gvData_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        else if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            UOM o = (UOM)oList[e.Row.RowIndex];
            cb_IsChecked.Enabled = oList.Exists(a => $"{((UOM)a).Mode}" != "") ? false : true;
    ...
    protected void ucPaging_OpChanged(object sender, EventArgs e)
    {
        ...
        List<object> oList = oList = UOM.GetByCriteria(pList, out double TotalRow);
    ...
    protected void wSearch_Close(object sender, F.WindowCloseEventArgs e)
    {
        ...
        btnSearch.OnClientClick = wSearch.GetShowReference($"~/Pages/Search.aspx?ClassName=UOM&SearchData={arrArgument[1].Replace(Environment.NewLine, "~")}");
    ...
    protected void wSort_Close(object sender, F.WindowCloseEventArgs e)
    {
        ...
        btnSorting.OnClientClick = wSort.GetShowReference($"~/Pages/Sort.aspx?ClassName=UOM&SortData={arrArgument[1].Replace(Environment.NewLine, "~")}");
    ...
    private void SetInit()
    {
        ...
        btnSearch.OnClientClick = $"{wSearch.GetShowReference($"~/Pages/Search.aspx?ClassName=UOM")}return false;";
        btnSorting.OnClientClick = $"{wSort.GetShowReference($"~/Pages/Sort.aspx?ClassName=UOM")}return false;";
        ViewState[CNT.VS.Table] = "UOM";
    }
    ...
    private void BindData()
    {
        ...
        List<object> oList = UOM.GetByCriteria(pList, out TotalRow);
    ...
    {
        oList.Add(new UOM() { No = -1, Name = CNT.DataNotAvailable });
    ...
    private void SetInitAdd()
    {
        ...
        List<object> oList = U.GetGridData(gvData, UOM.MyType).ListData.FindAll(a => ((UOM)a).Name != CNT.DataNotAvailable);
        oList.ForEach(a => { ((UOM)a).IsChecked = false; ((UOM)a).Mode = ""; });
        oList.Insert(0, new UOM() { No = 0, IsChecked = true, Mode = "add" });
        U.BindGrid(gvData, oList);
    ...
    {
        List<object> oList = U.GetGridData(gvData, UOM.MyType).ListData;
        if (!oList.Exists(a => ((UOM)a).IsChecked))
        {
        ...
        oList.ForEach(a => { ((UOM)a).Mode = ((UOM)a).IsChecked ? "edit" : ""; });
        U.BindGrid(gvData, oList);
        ((CheckBox)gvData.HeaderRow.Cells[0].FindControl("cbCheckAll")).Enabled = false;
    }
    private void Cancel()
    ...
    private void SaveAdd()
    {
        ...
        {
            string Result = "";
            List<object> oList = U.GetGridData(gvData, typeof(UOM)).ListData;
            UOM o = (UOM)oList[0];
            ...
            int Seq = 0;
            if (DataAccess.IsFieldExist(ViewState[CNT.VS.Table], "Code", o.Code, o.Id)) sb.AppendLine($"{Seq.Inc()}. UOM with Code : {o.Code} already exist");
            if (DataAccess.IsFieldExist(ViewState[CNT.VS.Table], "Name", o.Name, o.Id)) sb.AppendLine($"{Seq.Inc()}. UOM with Name : {o.Name} already exist");
    ...
    private void Delete()
    {
        try
        {
            string Result = "";
            List<object> oList = U.GetGridData(gvData, UOM.MyType).ListData;
            if (oList.FindAll(a => ((UOM)a).IsChecked).Count == 0)
            {
            ...
            for (int i = 0; i < oList.Count; i++)
            {
                ...
                Result = DataAccess.IsDataExist(0, "UOMId", o.Id);
                if (!Result.IsEmpty())
                {
    ...
    private void ExportExcel()
    {
        ...
        List<object> oList = UOM.GetByCriteria(pList, out double TotalRow);
        ...
        resp.Clear();
        resp.Buffer = false;
        resp.AppendHeader("content-disposition", string.Format("attachment; filename=UOM.xlsx"));
        ...
        byte[] buffer = null;
        string tempFilename = $@"{U.PathTempFolder}UOM.xlsx";
