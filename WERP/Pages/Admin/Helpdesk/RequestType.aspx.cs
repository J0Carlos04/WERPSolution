...
public partial class Pages_Admin_Helpdesk_RequestType : PageBase
{    
    #region Handlers
    ...
    protected void gvData_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        else if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            RequestType o = (RequestType)oList[e.Row.RowIndex];
            cb_IsChecked.Enabled = oList.Exists(a => $"{((RequestType)a).Mode}" != "") ? false : true;
    ...
    protected void ucPaging_OpChanged(object sender, EventArgs e)
    {
        ...
        List<object> oList = oList = RequestType.GetByCriteria(pList, out double TotalRow);
    ...
    protected void wSearch_Close(object sender, F.WindowCloseEventArgs e)
    {
        ...
        btnSearch.OnClientClick = wSearch.GetShowReference($"~/Pages/Search.aspx?ClassName=RequestType&SearchData={arrArgument[1].Replace(Environment.NewLine, "~")}");
    ...
    protected void wSort_Close(object sender, F.WindowCloseEventArgs e)
    {
        ...
        btnSorting.OnClientClick = wSort.GetShowReference($"~/Pages/Sort.aspx?ClassName=RequestType&SortData={arrArgument[1].Replace(Environment.NewLine, "~")}");
    ...
    private void SetInit()
    {
        ...
        btnSearch.OnClientClick = $"{wSearch.GetShowReference($"~/Pages/Search.aspx?ClassName=RequestType")}return false;";
        btnSorting.OnClientClick = $"{wSort.GetShowReference($"~/Pages/Sort.aspx?ClassName=RequestType")}return false;";
        ViewState[CNT.VS.Table] = "RequestType";
    }
    ...
    private void BindData()
    {
        ...
        List<object> oList = RequestType.GetByCriteria(pList, out TotalRow);
    ...
    {
        oList.Add(new RequestType() { No = -1, Name = CNT.DataNotAvailable });
    ...
    private void SetInitAdd()
    {
        ...
        List<object> oList = U.GetGridData(gvData, RequestType.MyType).ListData.FindAll(a => ((RequestType)a).Name != CNT.DataNotAvailable);
        oList.ForEach(a => { ((RequestType)a).IsChecked = false; ((RequestType)a).Mode = ""; });
        oList.Insert(0, new RequestType() { No = 0, IsChecked = true, Mode = "add" });
        U.BindGrid(gvData, oList);
    ...
    {
        List<object> oList = U.GetGridData(gvData, RequestType.MyType).ListData;
        if (!oList.Exists(a => ((RequestType)a).IsChecked))
        {
        ...
        oList.ForEach(a => { ((RequestType)a).Mode = ((RequestType)a).IsChecked ? "edit" : ""; });
        U.BindGrid(gvData, oList);
        ((CheckBox)gvData.HeaderRow.Cells[0].FindControl("cbCheckAll")).Enabled = false;
    }
    private void Cancel()
    ...
    private void SaveAdd()
    {
        try
        {
            string Result = "";
            List<object> oList = U.GetGridData(gvData, RequestType.MyType).ListData;
            RequestType o = (RequestType)oList[0];
            ...
            int Seq = 0;
            if (RequestType.IsFieldExist("Name", o.Name, o.Id)) sb.AppendLine($"{Seq.Inc()}. Request Type with Name : {o.Name} already exist");            
    ...
    private void SaveEdit()
    {
        ...
        int Seq = 0;
        StringBuilder sb = new StringBuilder();
        foreach (RequestType o in oList)
        {
    ...
    private void Delete()
    {
        try
        {
            string Result = "";
            List<object> oList = U.GetGridData(gvData, RequestType.MyType).ListData;
            if (oList.FindAll(a => ((RequestType)a).IsChecked).Count == 0)
            {
            ...
            {
                RequestType o = (RequestType)oList[i];
                if (!o.IsChecked) continue;

                Result = DataAccess.IsDataExist(0, "RequestTypeId", o.Id);
                if (!Result.IsEmpty())
                {
                ...
                Result = RequestType.Delete(o.Id);
                if (Result.Contains("Error Message"))
                {
    ...
    private void ExportExcel()
    {
        ...
        List<object> oList = RequestType.GetByCriteria(pList, out double TotalRow);
        Wrapping w = new Wrapping { Sheet = sheet, CellStyle = HeaderStyle };

        #region HeaderExcel	
        w.SetExcel("No");        
        w.SetExcel("Name");
        w.SetExcel("Response Time (Hours)");
        w.SetExcel("Active");
        #endregion
        ...
        resp.Clear();
        resp.Buffer = false;
        resp.AppendHeader("content-disposition", string.Format("attachment; filename=RequestType.xlsx"));
        ...
        byte[] buffer = null;
        string tempFilename = $@"{U.PathTempFolder}RequestType.xlsx";
