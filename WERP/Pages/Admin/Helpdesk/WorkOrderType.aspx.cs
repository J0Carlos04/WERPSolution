...
public partial class Pages_Admin_Helpdesk_WorkOrderType : PageBase
{    
    #region Handlers
    ...
    protected void gvData_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        else if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            WorkOrderType o = (WorkOrderType)oList[e.Row.RowIndex];
            cb_IsChecked.Enabled = oList.Exists(a => $"{((WorkOrderType)a).Mode}" != "") ? false : true;
    ...
    protected void ucPaging_OpChanged(object sender, EventArgs e)
    {
        ...
        List<object> oList = oList = WorkOrderType.GetByCriteria(pList, out double TotalRow);
    ...
    protected void wSearch_Close(object sender, F.WindowCloseEventArgs e)
    {
        ...
        btnSearch.OnClientClick = wSearch.GetShowReference($"~/Pages/Search.aspx?ClassName=WorkOrderType&SearchData={arrArgument[1].Replace(Environment.NewLine, "~")}");
    ...
    protected void wSort_Close(object sender, F.WindowCloseEventArgs e)
    {
        ...
        btnSorting.OnClientClick = wSort.GetShowReference($"~/Pages/Sort.aspx?ClassName=WorkOrderType&SortData={arrArgument[1].Replace(Environment.NewLine, "~")}");
    ...
    private void SetInit()
    {
        ...
        btnSearch.OnClientClick = $"{wSearch.GetShowReference($"~/Pages/Search.aspx?ClassName=WorkOrderType")}return false;";
        btnSorting.OnClientClick = $"{wSort.GetShowReference($"~/Pages/Sort.aspx?ClassName=WorkOrderType")}return false;";
        ViewState[CNT.VS.Table] = "WorkOrderType";
    }
    ...
    private void BindData()
    {
        ...
        List<object> oList = WorkOrderType.GetByCriteria(pList, out TotalRow);
    ...
    {
        oList.Add(new WorkOrderType() { No = -1, Name = CNT.DataNotAvailable });
    ...
    private void SetInitAdd()
    {
        ...
        List<object> oList = U.GetGridData(gvData, WorkOrderType.MyType).ListData.FindAll(a => ((WorkOrderType)a).Name != CNT.DataNotAvailable);
        oList.ForEach(a => { ((WorkOrderType)a).IsChecked = false; ((WorkOrderType)a).Mode = ""; });
        oList.Insert(0, new WorkOrderType() { No = 0, IsChecked = true, Mode = "add" });
        U.BindGrid(gvData, oList);
    ...
    {
        List<object> oList = U.GetGridData(gvData, WorkOrderType.MyType).ListData;
        if (!oList.Exists(a => ((WorkOrderType)a).IsChecked))
        {
        ...
        oList.ForEach(a => { ((WorkOrderType)a).Mode = ((WorkOrderType)a).IsChecked ? "edit" : ""; });
        U.BindGrid(gvData, oList);
        ((CheckBox)gvData.HeaderRow.Cells[0].FindControl("cbCheckAll")).Enabled = false;
    }
    private void Cancel()
    ...
    private void SaveAdd()
    {
        ...
        {
            string Result = "";
            List<object> oList = U.GetGridData(gvData, typeof(WorkOrderType)).ListData;
            WorkOrderType o = (WorkOrderType)oList[0];
            ...
            int Seq = 0;
            if (DataAccess.IsFieldExist(ViewState[CNT.VS.Table], "Code", o.Code, o.Id)) sb.AppendLine($"{Seq.Inc()}. Work Order Type with Code : {o.Code} already exist");
            if (DataAccess.IsFieldExist(ViewState[CNT.VS.Table], "Name", o.Name, o.Id)) sb.AppendLine($"{Seq.Inc()}. Work Order Type with Name : {o.Name} already exist");
    ...
    private void SaveEdit()
    {
        ...
        int Seq = 0;
        StringBuilder sb = new StringBuilder();
        foreach (WorkOrderType o in oList)
        {
    ...
    private void Delete()
    {
        try
        {
            string Result = "";
            List<object> oList = U.GetGridData(gvData, WorkOrderType.MyType).ListData;
            if (oList.FindAll(a => ((WorkOrderType)a).IsChecked).Count == 0)
            {
            ...
            {
                WorkOrderType o = (WorkOrderType)oList[i];
                if (!o.IsChecked) continue;

                Result = DataAccess.IsDataExist(0, "WorkOrderTypeId", o.Id);
                if (!Result.IsEmpty())
                {
                ...
                Result = WorkOrderType.Delete(o.Id);
                if (Result.Contains("Error Message"))
                {
    ...
    private void ExportExcel()
    {
        ...
        List<object> oList = WorkOrderType.GetByCriteria(pList, out double TotalRow);
        ...
        resp.Clear();
        resp.Buffer = false;
        resp.AppendHeader("content-disposition", string.Format("attachment; filename=WorkOrderType.xlsx"));
        ...
        byte[] buffer = null;
        string tempFilename = $@"{U.PathTempFolder}WorkOrderType.xlsx";
