<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Subject.aspx.cs" Inherits="Pages_Admin_Subject" %>
...
<f:Panel ID="pContent" runat="server" ShowBorder="false" ShowHeader="false" Layout="Region" EnableAjaxLoading="true" AjaxLoadingType="Default">
  <Toolbars>
    <f:Toolbar runat="server" ID="tTop" Position="Top"><Items>
      ...
      <f:Button runat="server" ID="btnDelete" Text="Delete" Icon="Delete" OnClick="btn_Click" ConfirmText="Are you sure you want to delete this Subject?. This action cannot be undone. Do you wish to proceed?" ConfirmIcon="Warning" />
        ...
        <asp:GridView runat="server" ID="gvData" AutoGenerateColumns="false" CssClass="table table-striped table-bordered table-hover" OnRowDataBound="gvData_RowDataBound" Width="100%" HeaderStyle-HorizontalAlign="Center">
          <Columns>
            ...
            <asp:TemplateField HeaderText="Code" >              
              <ItemTemplate>		    
                <asp:Literal runat="server" ID="ltrlCode" Text='<%# Eval("Code") %>' Visible='<%# Eval("Mode").IsEmpty() %>' />                        
                <asp:TextBox runat="server" ID="tb_Code" Text='<%# $"{Eval("Mode")}" == "add" ? "" : $"{Eval("Code")}" %>' Visible='<%# !Eval("Mode").IsEmpty() %>' CssClass="form-control form-control-sm" MaxLength="20" onkeypress="return NoSpace(event);" />
              </ItemTemplate>
            </asp:TemplateField>
            ...
            <asp:TemplateField HeaderText="Need Cust No" ><ItemTemplate>		    
              <asp:Literal runat="server" ID="ltrlNeedCustNo" Text='<%# $"{Eval("NeedCustNo")}" == "True" ? "Yes" : "No" %>' Visible='<%# $"{Eval("Mode")}" == "" && $"{Eval("No")}" != "-1" ? true : false %>' />                        
              <asp:CheckBox runat="server" ID="cb_NeedCustNo" Checked='<%# Eval("NeedCustNo") %>' Visible='<%# !Eval("Mode").IsEmpty() %>' CssClass="cb" />
            </ItemTemplate></asp:TemplateField> 

            <asp:TemplateField HeaderText="Use Item" ><ItemTemplate>		    
              <asp:Literal runat="server" ID="ltrlUseItem" Text='<%# Eval("UseItem") %>' Visible='<%# $"{Eval("Mode")}" == "" && $"{Eval("No")}" != "-1" ? true : false %>' />                        
              <asp:DropDownList runat="server" ID="ddl_UseItem" Visible='<%# !Eval("Mode").IsEmpty() %>' CssClass="form-select form-select-sm" >
            ...
            <asp:TemplateField HeaderText="Work Duration" ><ItemTemplate>		    
              <asp:Literal runat="server" ID="ltrlWorkDuration" Text='<%# $"{Eval("WorkDuration")} {($"{Eval("WorkDurationType")}" == "" ? "" : $"{Eval("WorkDurationType")}s" )}" %>' Visible='<%# $"{Eval("Mode")}" == "" && $"{Eval("No")}" != "-1" ? true : false %>' />                        
              <table><tr>
                <td><asp:TextBox runat="server" ID="tb_WorkDuration" type="number" Text='<%# Eval("WorkDuration")  %>' Visible='<%# !Eval("Mode").IsEmpty() %>' CssClass="form-control form-control-sm" Width="70" /></td>
                <td>
                  <asp:DropDownList runat="server" ID="ddl_WorkDurationType" class="form-select form-select-sm" Visible='<%# !Eval("Mode").IsEmpty() %>' Width="100" >
            ...
            <asp:TemplateField HeaderText="Use NRW" ><ItemTemplate>		    
              <asp:Literal runat="server" ID="ltrlUseNRW" Text='<%# $"{Eval("UseNRW")}" == "True" ? "Yes" : "No" %>' Visible='<%# $"{Eval("Mode")}" == "" && $"{Eval("No")}" != "-1" ? true : false %>' />                        
              <asp:CheckBox runat="server" ID="cb_UseNRW" Checked='<%# Eval("UseNRW") %>' Visible='<%# !Eval("Mode").IsEmpty() %>' CssClass="cb" />
            </ItemTemplate></asp:TemplateField>
            <asp:TemplateField HeaderText="Meter&nbsp;Condition" ><ItemTemplate>		    
              <asp:Literal runat="server" ID="ltrlMeterCondition" Text='<%# $"{Eval("MeterCondition")}" == "True" ? "Yes" : "No" %>' Visible='<%# $"{Eval("Mode")}" == "" && $"{Eval("No")}" != "-1" ? true : false %>' />                        
              <asp:CheckBox runat="server" ID="cb_MeterCondition" Checked='<%# Eval("MeterCondition") %>' Visible='<%# !Eval("Mode").IsEmpty() %>' CssClass="cb" />
            </ItemTemplate></asp:TemplateField>
            <asp:TemplateField HeaderText="Use DPD" ><ItemTemplate>		    
              <asp:Literal runat="server" ID="ltrlUseDPD" Text='<%# $"{Eval("UseDPD")}" == "True" ? "Yes" : "No" %>' Visible='<%# $"{Eval("Mode")}" == "" && $"{Eval("No")}" != "-1" ? true : false %>' />                        
              <asp:CheckBox runat="server" ID="cb_UseDPD" Checked='<%# Eval("UseDPD") %>' Visible='<%# !Eval("Mode").IsEmpty() %>' CssClass="cb" />
            </ItemTemplate></asp:TemplateField>
            <asp:TemplateField HeaderText="Is Leak" ><ItemTemplate>		    
              <asp:Literal runat="server" ID="ltrlLeak" Text='<%# $"{Eval("Leak")}" == "True" ? "Yes" : "No" %>' Visible='<%# $"{Eval("Mode")}" == "" && $"{Eval("No")}" != "-1" ? true : false %>' />                        
              <asp:CheckBox runat="server" ID="cb_Leak" Checked='<%# Eval("Leak") %>' Visible='<%# !Eval("Mode").IsEmpty() %>' CssClass="cb" />
            </ItemTemplate></asp:TemplateField>
            <asp:TemplateField HeaderText="Need Restoration" ><ItemTemplate>		    
              <asp:Literal runat="server" ID="ltrlNeedRestoration" Text='<%# $"{Eval("NeedRestoration")}" == "True" ? "Yes" : "No" %>' Visible='<%# $"{Eval("Mode")}" == "" && $"{Eval("No")}" != "-1" ? true : false %>' />                        
              <asp:CheckBox runat="server" ID="cb_NeedRestoration" Checked='<%# Eval("NeedRestoration") %>' Visible='<%# !Eval("Mode").IsEmpty() %>' CssClass="cb" />
            </ItemTemplate></asp:TemplateField>
            <asp:TemplateField HeaderText="Need Closed" ><ItemTemplate>		    
              <asp:Literal runat="server" ID="ltrlNeedClosed" Text='<%# $"{Eval("NeedClosed")}" == "True" ? "Yes" : "No" %>' Visible='<%# $"{Eval("Mode")}" == "" && $"{Eval("No")}" != "-1" ? true : false %>' />                        
              <asp:CheckBox runat="server" ID="cb_NeedClosed" Checked='<%# Eval("NeedClosed") %>' Visible='<%# !Eval("Mode").IsEmpty() %>' CssClass="cb" />
            </ItemTemplate></asp:TemplateField>
            <asp:TemplateField HeaderText="Use Time Limit" ><ItemTemplate>		    
              <asp:Literal runat="server" ID="ltrlUseTimeLimit" Text='<%# $"{Eval("UseTimeLimit")}" == "True" ? "Yes" : "No" %>' Visible='<%# $"{Eval("Mode")}" == "" && $"{Eval("No")}" != "-1" ? true : false %>' />                        
              <asp:CheckBox runat="server" ID="cb_UseTimeLimit" Checked='<%# Eval("UseTimeLimit") %>' Visible='<%# !Eval("Mode").IsEmpty() %>' CssClass="cb" />
            </ItemTemplate></asp:TemplateField>                        
            <asp:TemplateField HeaderText="Use Photo" ><ItemTemplate>		    
              <asp:Literal runat="server" ID="ltrlUsePhoto" Text='<%# $"{Eval("UsePhoto")}" == "True" ? "Yes" : "No" %>' Visible='<%# $"{Eval("Mode")}" == "" && $"{Eval("No")}" != "-1" ? true : false %>' />                        
              <asp:CheckBox runat="server" ID="cb_UsePhoto" Checked='<%# Eval("UsePhoto") %>' Visible='<%# !Eval("Mode").IsEmpty() %>' CssClass="cb" />
            </ItemTemplate></asp:TemplateField>
            <asp:TemplateField HeaderText="Use Section" ><ItemTemplate>		    
              <asp:Literal runat="server" ID="ltrlUseSection" Text='<%# $"{Eval("UseSection")}" == "True" ? "Yes" : "No" %>' Visible='<%# $"{Eval("Mode")}" == "" && $"{Eval("No")}" != "-1" ? true : false %>' />                        
              <asp:CheckBox runat="server" ID="cb_UseSection" Checked='<%# Eval("UseSection") %>' Visible='<%# !Eval("Mode").IsEmpty() %>' CssClass="cb" />
            </ItemTemplate></asp:TemplateField>
            <asp:TemplateField HeaderText="Update GIS" ><ItemTemplate>		    
              <asp:Literal runat="server" ID="ltrlUpdateGIS" Text='<%# $"{Eval("UpdateGIS")}" == "True" ? "Yes" : "No" %>' Visible='<%# $"{Eval("Mode")}" == "" && $"{Eval("No")}" != "-1" ? true : false %>' />                        
              <asp:CheckBox runat="server" ID="cb_UpdateGIS" Checked='<%# Eval("UpdateGIS") %>' Visible='<%# !Eval("Mode").IsEmpty() %>' CssClass="cb" />
            </ItemTemplate></asp:TemplateField>
