<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Location.aspx.cs" Inherits="Pages_Admin_Location" %>
    ...
    <f:Panel ID="pContent" runat="server" ShowBorder="false" ShowHeader="false" Layout="Region" EnableAjaxLoading="true" AjaxLoadingType="Default">
      <Toolbars>
        <f:Toolbar runat="server" ID="tTop" Position="Top"><Items>
          ...
          <f:Button runat="server" ID="btnDelete" Text="Delete" Icon="Delete" OnClick="btn_Click" ConfirmText="Are you sure you want to delete this Location?. This action cannot be undone. Do you wish to proceed?" ConfirmIcon="Warning" />
            ...
            <asp:GridView runat="server" ID="gvData" AutoGenerateColumns="false" CssClass="table table-striped table-bordered table-hover" OnRowDataBound="gvData_RowDataBound" Width="100%" HeaderStyle-HorizontalAlign="Center">
              <Columns>
                ...
                <asp:TemplateField HeaderText="Area">
                  <ItemTemplate>
                    <asp:Literal runat="server" ID="ltrl_Area" Text='<%# Eval("Area") %>' Visible='<%# Eval("Mode").IsEmpty() %>' />
                    <asp:DropDownList runat="server" ID="ddl_AreaId" Visible='<%# !Eval("Mode").IsEmpty() %>' CssClass="form-select form-select-sm" />
                  </ItemTemplate>
                </asp:TemplateField>

                <asp:TemplateField HeaderText="Type">
                  <ItemTemplate>
                    <asp:Literal runat="server" ID="ltrlType" Text='<%# Eval("Type") %>' Visible='<%# Eval("Mode").IsEmpty() %>' />
                    <asp:DropDownList runat="server" ID="ddl_Type" AutoPostBack="true" OnSelectedIndexChanged="ddl_SelectedIndexChanged" Visible='<%# !Eval("Mode").IsEmpty() %>' CssClass="form-select form-select-sm" />                   
                  </ItemTemplate>
                </asp:TemplateField>
                ...
                <asp:TemplateField HeaderText="Name">
                  <ItemTemplate>
                    ...
                    <asp:TextBox runat="server" ID="tb_Name" Text='<%# $"{Eval("Mode")}" == "add" ? "" : $"{Eval("Name")}" %>' Visible='<%# Eval("Mode").IsNotEmpty() && Eval("Type").Is("Customer") %>' CssClass="form-control form-control-sm" />
                    <asp:DropDownList runat="server" ID="ddl_FunctionStationId" Visible='<%# !Eval("Mode").IsEmpty() %>' CssClass="form-select form-select-sm" />
                  </ItemTemplate>
                </asp:TemplateField>
                ...
                <asp:TemplateField HeaderText="Address">
                  <ItemTemplate>
                    ...
                    <asp:TextBox runat="server" ID="tb_Address" Text='<%# $"{Eval("Mode")}" == "add" ? "" : $"{Eval("Address")}" %>' Visible='<%# Eval("Mode").IsNotEmpty() %>' CssClass="form-control form-control-sm" />                    
                  </ItemTemplate>
                </asp:TemplateField>

                <asp:TemplateField HeaderText="RT">
                  <ItemTemplate>
                    <asp:Literal runat="server" ID="ltrlRT" Text='<%# Eval("RT") %>' Visible='<%# Eval("Mode").IsEmpty() %>' />
                    <asp:TextBox runat="server" ID="tb_RT" Text='<%# $"{Eval("Mode")}" == "add" ? "" : $"{Eval("RT")}" %>' Visible='<%# Eval("Mode").IsNotEmpty() %>' CssClass="form-control form-control-sm" />
                  </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="RW">
                  <ItemTemplate>
                    <asp:Literal runat="server" ID="ltrlRW" Text='<%# Eval("RW") %>' Visible='<%# Eval("Mode").IsEmpty() %>' />
                    <asp:TextBox runat="server" ID="tb_RW" Text='<%# $"{Eval("Mode")}" == "add" ? "" : $"{Eval("RW")}" %>' Visible='<%# Eval("Mode").IsNotEmpty() %>' CssClass="form-control form-control-sm" />
                  </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="UrbanVilage">
                  <ItemTemplate>
                    <asp:Literal runat="server" ID="ltrlUrbanVilage" Text='<%# Eval("UrbanVilage") %>' Visible='<%# Eval("Mode").IsEmpty() %>' />
                    <asp:TextBox runat="server" ID="tb_UrbanVilage" Text='<%# $"{Eval("Mode")}" == "add" ? "" : $"{Eval("UrbanVilage")}" %>' Visible='<%# Eval("Mode").IsNotEmpty() %>' CssClass="form-control form-control-sm" />
                  </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="SubDistrict">
                  <ItemTemplate>
                    <asp:Literal runat="server" ID="ltrlSubDistrict" Text='<%# Eval("SubDistrict") %>' Visible='<%# Eval("Mode").IsEmpty() %>' />
                    <asp:TextBox runat="server" ID="tb_SubDistrict" Text='<%# $"{Eval("Mode")}" == "add" ? "" : $"{Eval("SubDistrict")}" %>' Visible='<%# Eval("Mode").IsNotEmpty() %>' CssClass="form-control form-control-sm" />                    
                  </ItemTemplate>
                </asp:TemplateField>
                
                <asp:TemplateField HeaderText="Longitude">
                  <ItemTemplate>
                    <asp:Literal runat="server" ID="ltrlLongitude" Text='<%# NormalizeDecimal(Eval("Longitude")) %>' Visible='<%# $"{Eval("Mode")}" == "" && $"{Eval("No")}" != "-1" ? true : false %>' />
                    <asp:TextBox runat="server" ID="tb_Longitude" Text='<%# $"{Eval("Mode")}" == "add" ? "" : $"{Eval("Longitude")}" %>' TextMode="Number" Rows="1" Visible='<%# !Eval("Mode").IsEmpty() %>' CssClass="form-control form-control-sm" />
                  </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="Latitude">
                  <ItemTemplate>
                    <asp:Literal runat="server" ID="ltrlLatitude" Text='<%# NormalizeDecimal(Eval("Latitude")) %>' Visible='<%# $"{Eval("Mode")}" == "" && $"{Eval("No")}" != "-1" ? true : false %>' />
                    <asp:TextBox runat="server" ID="tb_Latitude" Text='<%# $"{Eval("Mode")}" == "add" ? "" : $"{Eval("Latitude")}" %>' TextMode="Number" Rows="1" Visible='<%# !Eval("Mode").IsEmpty() %>' CssClass="form-control form-control-sm" />
                  </ItemTemplate>
                </asp:TemplateField>
