...
public partial class Pages_Vendor : PageBase
{    
    #region Handlers
    ...
    protected void gvData_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        else if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            Vendor o = (Vendor)oList[e.Row.RowIndex];
            cb_IsChecked.Enabled = oList.Exists(a => $"{((Vendor)a).Mode}" != "") ? false : true;
    ...
    protected void ucPaging_OpChanged(object sender, EventArgs e)
    {
        ...
        List<object> oList = oList = Vendor.GetByCriteria(pList, out double TotalRow);
    ...
    protected void wSearch_Close(object sender, F.WindowCloseEventArgs e)
    {
        ...
        btnSearch.OnClientClick = wSearch.GetShowReference($"~/Pages/Search.aspx?ClassName=Vendor&SearchData={arrArgument[1].Replace(Environment.NewLine, "~")}");
    ...
    protected void wSort_Close(object sender, F.WindowCloseEventArgs e)
    {
        ...
        btnSorting.OnClientClick = wSort.GetShowReference($"~/Pages/Sort.aspx?ClassName=Vendor&SortData={arrArgument[1].Replace(Environment.NewLine, "~")}");
    ...
    private void SetInit()
    {
        ...
        btnSearch.OnClientClick = $"{wSearch.GetShowReference($"~/Pages/Search.aspx?ClassName=Vendor")}return false;";
        btnSorting.OnClientClick = $"{wSort.GetShowReference($"~/Pages/Sort.aspx?ClassName=Vendor")}return false;";
        ViewState[CNT.VS.Table] = "Vendor";
    }    
    ...
    private void BindData()
    {
        ...
        List<object> oList = Vendor.GetByCriteria(pList, out TotalRow);
    ...
    {
        oList.Add(new Vendor() { No = -1, Name = CNT.DataNotAvailable });
    ...
    private void SetInitAdd()
    {
        ...
        List<object> oList = U.GetGridData(gvData, Vendor.MyType).ListData.FindAll(a => ((Vendor)a).Name != CNT.DataNotAvailable);
        oList.ForEach(a => { ((Vendor)a).IsChecked = false; ((Vendor)a).Mode = ""; });
        oList.Insert(0, new Vendor() { No = 0, IsChecked = true, Mode = "add" });
        U.BindGrid(gvData, oList);
    ...
    {
        List<object> oList = U.GetGridData(gvData, Vendor.MyType).ListData;
        if (!oList.Exists(a => ((Vendor)a).IsChecked))
        {
        ...
        oList.ForEach(a => { ((Vendor)a).Mode = ((Vendor)a).IsChecked ? "edit" : ""; });
        U.BindGrid(gvData, oList);
        ((CheckBox)gvData.HeaderRow.Cells[0].FindControl("cbCheckAll")).Enabled = false;
    }
    private void Cancel()
    ...
    private void SaveAdd()
    {
        ...
        {
            string Result = "";
            List<object> oList = U.GetGridData(gvData, typeof(Vendor)).ListData;
            Vendor o = (Vendor)oList[0];
            ...
            int Seq = 0;
            if (DataAccess.IsFieldExist(ViewState[CNT.VS.Table], "Code", o.Code, o.Id)) sb.AppendLine($"{Seq.Inc()}. Vendor with Code : {o.Code} already exist");
            if (DataAccess.IsFieldExist(ViewState[CNT.VS.Table], "Name", o.Name, o.Id)) sb.AppendLine($"{Seq.Inc()}. Vendor with Name : {o.Name} already exist");
    ...
    private void Delete()
    {
        try
        {
            string Result = "";
            List<object> oList = U.GetGridData(gvData, Vendor.MyType).ListData;
            if (oList.FindAll(a => ((Vendor)a).IsChecked).Count == 0)
            {
            ...
            {
                Vendor o = (Vendor)oList[i];
                if (!o.IsChecked) continue;

                Result = DataAccess.IsDataExist(0, "VendorId", o.Id);
                if (!Result.IsEmpty())
                {
    ...
    private void ExportExcel()
    {
        ...
        List<object> oList = Vendor.GetByCriteria(pList, out double TotalRow);
        ...
        resp.Clear();
        resp.Buffer = false;
        resp.AppendHeader("content-disposition", string.Format("attachment; filename=Vendor.xlsx"));
        ...
        byte[] buffer = null;
        string tempFilename = $@"{U.PathTempFolder}Vendor.xlsx";
