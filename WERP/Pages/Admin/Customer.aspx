<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Customer.aspx.cs" Inherits="Pages_Admin_Customer" %>
...
<f:Panel ID="pContent" runat="server" ShowBorder="false" ShowHeader="false" Layout="Region" EnableAjaxLoading="true" AjaxLoadingType="Default">
  <Toolbars>
    <f:Toolbar runat="server" ID="tTop" Position="Top"><Items>
      ...
      <f:Button runat="server" ID="btnDelete" Text="Delete" Icon="Delete" OnClick="btn_Click" ConfirmText="Are you sure you want to delete this record?. This action cannot be undone. Do you wish to proceed?" ConfirmIcon="Warning"  />
        ...
        <asp:GridView runat="server" ID="gvData" AutoGenerateColumns="false" CssClass="table table-striped table-bordered table-hover" OnRowDataBound="gvData_RowDataBound" Width="100%" HeaderStyle-HorizontalAlign="Center">
          <Columns>
            ...
            <asp:TemplateField HeaderText="Location" >              
              <ItemTemplate>
                <div style="display:flex;">
                  <div style="flex-grow:1; padding-right:3px;">
                    <asp:Literal runat="server" ID="ltrl_LocationId" Text='<%# Eval("LocationId") %>' Visible="false" />  
                    <asp:Literal runat="server" ID="ltrlLocation" Text='<%# Eval("Location") %>' Visible='<%# Eval("Mode").IsEmpty() %>' />                        
                    <asp:TextBox runat="server" ID="tb_Location" Text='<%# $"{Eval("Mode")}" == "add" ? "" : $"{Eval("Location")}" %>'  Visible='<%# !Eval("Mode").IsEmpty() %>' CssClass="form-control form-control-sm" />
                  </div>
                  <div style="padding-left:3px;" ><asp:LinkButton runat="server" ID="lbLocation" Text="..." Style="margin-top:3px; font-size:30px; text-decoration:none;" OnClick="lb_Click" CommandArgument='<%# Eval("No") %>' Visible='<%# !Eval("Mode").IsEmpty() %>' /></div>
                </div>
              </ItemTemplate>
            </asp:TemplateField>
            <asp:TemplateField HeaderText="Cust&nbsp;No" >              
              <ItemTemplate>		    
                <asp:Literal runat="server" ID="ltrlCustNo" Text='<%# Eval("CustNo") %>' Visible='<%# Eval("Mode").IsEmpty() %>' />                        
                <asp:TextBox runat="server" ID="tb_CustNo" Text='<%# $"{Eval("Mode")}" == "add" ? "" : $"{Eval("CustNo")}" %>'  Visible='<%# !Eval("Mode").IsEmpty() %>' CssClass="form-control form-control-sm" />
              </ItemTemplate>
            </asp:TemplateField> 
            ...
            <asp:TemplateField HeaderText="Address" >              
              <ItemTemplate>		    
                <asp:Literal runat="server" ID="ltrlAddress" Text='<%# Eval("Address") %>' Visible='<%# Eval("Mode").IsEmpty() %>' />                        
                <asp:TextBox runat="server" ID="tb_Address" Text='<%# $"{Eval("Mode")}" == "add" ? "" : $"{Eval("Address")}" %>'  Visible='<%# !Eval("Mode").IsEmpty() %>' TextMode="MultiLine" Rows="3" CssClass="form-control form-control-sm" />
              </ItemTemplate>
            </asp:TemplateField>
            <asp:TemplateField HeaderText="Phone" >              
              <ItemTemplate>		    
                <asp:Literal runat="server" ID="ltrlPhone" Text='<%# Eval("Phone") %>' Visible='<%# Eval("Mode").IsEmpty() %>' />                        
                <asp:TextBox runat="server" ID="tb_Phone" Text='<%# $"{Eval("Mode")}" == "add" ? "" : $"{Eval("Phone")}" %>'  Visible='<%# !Eval("Mode").IsEmpty() %>' CssClass="form-control form-control-sm" TextMode="Number" />
              </ItemTemplate>
            </asp:TemplateField>
            <asp:TemplateField HeaderText="Email" >              
              <ItemTemplate>		    
                <asp:Literal runat="server" ID="ltrlEmail" Text='<%# Eval("Email") %>' Visible='<%# Eval("Mode").IsEmpty() %>' />                        
                <asp:TextBox runat="server" ID="tb_Email" Text='<%# $"{Eval("Mode")}" == "add" ? "" : $"{Eval("Email")}" %>'  Visible='<%# !Eval("Mode").IsEmpty() %>' CssClass="form-control form-control-sm" />
              </ItemTemplate>
            </asp:TemplateField> 
            <asp:TemplateField HeaderText="Rate" >              
              <ItemTemplate>		    
                <asp:Literal runat="server" ID="ltrlRate" Text='<%# Eval("Rate") %>' Visible='<%# Eval("Mode").IsEmpty() %>' />                        
                <asp:DropDownList runat="server" ID="ddl_RateId" Visible='<%# !Eval("Mode").IsEmpty() %>' CssClass="form-select form-select-sm" />
              </ItemTemplate>
            </asp:TemplateField>             
            <asp:TemplateField HeaderText="Meter&nbsp;Seal&nbsp;No" >              
              <ItemTemplate>		    
                <asp:Literal runat="server" ID="ltrlMeterSealNo" Text='<%# Eval("MeterSealNo") %>' Visible='<%# Eval("Mode").IsEmpty() %>' />                        
                <asp:TextBox runat="server" ID="tb_MeterSealNo" Text='<%# $"{Eval("Mode")}" == "add" ? "" : $"{Eval("MeterSealNo")}" %>'  Visible='<%# !Eval("Mode").IsEmpty() %>' CssClass="form-control form-control-sm" />
              </ItemTemplate>
            </asp:TemplateField>
            <asp:TemplateField HeaderText="Meter&nbsp;Installation&nbsp;Date" >              
              <ItemTemplate>		    
                <asp:Literal runat="server" ID="ltrlMeterInstallationDate" Text='<%# $"{Eval("MeterInstallationDate")}" == "1/1/0001 12:00:00 AM" ? "" : $"{Eval("MeterInstallationDate", "{0: dd-MMM-yyyy}")}" %>' Visible='<%# Eval("Mode").IsEmpty() %>' />                        
                <asp:TextBox runat="server" ID="tb_MeterInstallationDate" Text='<%# $"{Eval("Mode")}" == "add" ? "" : $"{Eval("MeterInstallationDate")}" %>'  Visible='<%# !Eval("Mode").IsEmpty() %>' CssClass="form-control form-control-sm" TextMode="Date" />
              </ItemTemplate>
            </asp:TemplateField> 
...
<f:Window ID="wLocation" Title="Select Location" Hidden="true" EnableIFrame="true" runat="server" EnableMaximize="true" EnableResize="true" Target="Top" IsModal="false" Width="562px" Height="470px" OnClose="wLocation_Close"></f:Window>  
</form>
</body>
</html>
