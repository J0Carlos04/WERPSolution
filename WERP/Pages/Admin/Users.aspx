<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Users.aspx.cs" Inherits="Pages_Admin_Users" %>
...
<f:Panel ID="pContent" runat="server" ShowBorder="false" ShowHeader="false" Layout="Region" EnableAjaxLoading="true" AjaxLoadingType="Default">
  <Toolbars>
    <f:Toolbar runat="server" ID="tTop" Position="Top"><Items>
      ...
      <f:Button runat="server" ID="btnDelete" Text="Delete" Icon="Delete" OnClick="btn_Click" ConfirmText="Are you sure you want to delete this Users?. This action cannot be undone. Do you wish to proceed?" ConfirmIcon="Warning"  />
        ...
        <asp:GridView runat="server" ID="gvData" AutoGenerateColumns="false" CssClass="table table-striped table-bordered table-hover" OnRowDataBound="gvData_RowDataBound" Width="100%" HeaderStyle-HorizontalAlign="Center">
          <Columns>
            ...
            <asp:TemplateField HeaderText="Department" >              
              <ItemTemplate>		    
                <asp:Literal runat="server" ID="ltrl_Department" Text='<%# Eval("Department") %>' Visible='<%# Eval("Mode").IsEmpty() %>' />                        
                <asp:DropDownList runat="server" ID="ddl_DepartmentId" Visible='<%# !Eval("Mode").IsEmpty() %>' CssClass="form-select form-select-sm" />
              </ItemTemplate>
            </asp:TemplateField>
...
<f:Window ID="wSort" Title="Sorting" Hidden="true" EnableIFrame="true" runat="server" EnableMaximize="true" EnableResize="true" Target="Top" IsModal="false" Width="562px" Height="470px" OnClose="wSort_Close"></f:Window>                  
  ...
  <script>
        function getNewWindowUrl() {
            return F.baseUrl + 'Pages/admin/UsersInput.aspx?parenttabid=' + parent.getActiveTabId();
        }
        function getEditWindowUrl(id) {
            return F.baseUrl + 'Pages/admin/UsersInput.aspx?Id=' + id + '&parenttabid=' + parent.getActiveTabId();
        }
