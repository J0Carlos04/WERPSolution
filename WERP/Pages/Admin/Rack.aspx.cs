...
public partial class Pages_Rack : PageBase
{    
    #region Handlers
    ...
    protected void gvData_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        else if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            Rack o = (Rack)oList[e.Row.RowIndex];
            cb_IsChecked.Enabled = oList.Exists(a => $"{((Rack)a).Mode}" != "") ? false : true;
            if (o.WarehouseId != 0) ddl_WarehouseId.SelectedValue = o.WarehouseId.ToString();
    ...
    protected void ucPaging_OpChanged(object sender, EventArgs e)
    {
        ...
        List<object> oList = oList = Rack.GetByCriteria(pList, out double TotalRow);
    ...
    protected void wSearch_Close(object sender, F.WindowCloseEventArgs e)
    {
        ...
        btnSearch.OnClientClick = wSearch.GetShowReference($"~/Pages/Search.aspx?ClassName=Rack&SearchData={arrArgument[1].Replace(Environment.NewLine, "~")}");
    ...
    protected void wSort_Close(object sender, F.WindowCloseEventArgs e)
    {
        ...
        btnSorting.OnClientClick = wSort.GetShowReference($"~/Pages/Sort.aspx?ClassName=Rack&SortData={arrArgument[1].Replace(Environment.NewLine, "~")}");
    ...
    private void SetInit()
    {
        ...
        btnSearch.OnClientClick = $"{wSearch.GetShowReference($"~/Pages/Search.aspx?ClassName=Rack")}return false;";
        btnSorting.OnClientClick = $"{wSort.GetShowReference($"~/Pages/Sort.aspx?ClassName=Rack")}return false;";        
    }
    ...
    private void WarehouseChange(DropDownList ddl)
    {
        Literal ltrl_Warehouse = (Literal)ddl.FindControl("ltrl_Warehouse");
        ltrl_Warehouse.Text = ddl.SelectedItem.Text;
    }
    ...
    private void BindData()
    {
        ...
        List<object> oList = Rack.GetByCriteria(pList, out TotalRow);
    ...
    {
        oList.Add(new Rack() { No = -1, Name = CNT.DataNotAvailable });
    ...
    private void SetInitAdd()
    {
        ...
        List<object> oList = U.GetGridData(gvData, Rack.MyType).ListData.FindAll(a => ((Rack)a).Name != CNT.DataNotAvailable);
        oList.ForEach(a => { ((Rack)a).IsChecked = false; ((Rack)a).Mode = ""; });
        oList.Insert(0, new Rack() { No = 0, IsChecked = true, Mode = "add" });
        U.BindGrid(gvData, oList);
    ...
    {
        List<object> oList = U.GetGridData(gvData, Rack.MyType).ListData;
        if (!oList.Exists(a => ((Rack)a).IsChecked))
        {
        ...
        oList.ForEach(a => { ((Rack)a).Mode = ((Rack)a).IsChecked ? "edit" : ""; });
        U.BindGrid(gvData, oList);
        ((CheckBox)gvData.HeaderRow.Cells[0].FindControl("cbCheckAll")).Enabled = false;
    }
    private void Cancel()
    ...
    private void SaveAdd()
    {
        ...
        {
            string Result = "";
            List<object> oList = U.GetGridData(gvData, typeof(Rack)).ListData;
            Rack o = (Rack)oList[0];
            ...
            Rack oTemp = Rack.GetByKey(o.Name, o.WarehouseId);
            if (oTemp.Id != 0)
            {
                U.ShowMessage($"Save Failed, {Environment.NewLine}Rack with Name {o.Name} in Warehouse {o.Warehouse} already exist");
                return;
            }
            #endregion
    ...
    private void SaveEdit()
    {
        ...
        foreach (Rack o in oList)
        {
            ...
            Rack oTemp = Rack.GetByKey(o.Name, o.WarehouseId, o.Id);
            if (oTemp.Id != 0)
            {
                sb.AppendLine($"{Seq.Inc()}. Rack with Name {o.Name} in Warehouse {o.Warehouse} already exist");                
                return;
            }
    ...
    private void Delete()
    {
        try
        {
            string Result = "";
            List<object> oList = U.GetGridData(gvData, Rack.MyType).ListData;
            if (oList.FindAll(a => ((Rack)a).IsChecked).Count == 0)
            {
            ...
            for (int i = 0; i < oList.Count; i++)
            {
                ...
                Result = DataAccess.IsDataExist(0, "RackId", o.Id);
                if (!Result.IsEmpty())
                {
    ...
    private void ExportExcel()
    {
        ...
        List<object> oList = Rack.GetByCriteria(pList, out double TotalRow);
        Wrapping w = new Wrapping { Sheet = sheet, CellStyle = HeaderStyle };

        #region HeaderExcel	
        w.SetExcel("No");
        w.SetExcel("Warehouse");
        w.SetExcel("Name");
        w.SetExcel("Active");
        #endregion
        ...
        resp.Clear();
        resp.Buffer = false;
        resp.AppendHeader("content-disposition", string.Format("attachment; filename=Rack.xlsx"));
        ...
        byte[] buffer = null;
        string tempFilename = $@"{U.PathTempFolder}Rack.xlsx";
