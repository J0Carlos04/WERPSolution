<%@ Page Language="C#" AutoEventWireup="true" CodeFile="WorkOrder.aspx.cs" Inherits="Pages_WorkOrder_WorkOrder" %>
          ...
          <Columns>
            ...
            <asp:TemplateField HeaderText="Action" ItemStyle-HorizontalAlign="Center"><ItemTemplate>
              <div style="display:flex; justify-content:center; column-gap:4px;">
                <asp:LinkButton runat="server" ID="lbEdit" Text="Edit" CommandArgument='<%# Eval("Id")  %>' CommandName='<%# Eval("Code")  %>' OnClick="lb_Click"  Font-Underline="false" />
                <asp:LinkButton runat="server" ID="lbStockOut" Text="StockOut" CommandArgument='<%# Eval("Id")  %>' CommandName='<%# Eval("Code")  %>' OnClick="lb_Click"  Font-Underline="false" />                
              </div> 
            </ItemTemplate></asp:TemplateField>
            
            <asp:TemplateField HeaderText="Code" ><ItemTemplate><div style="min-width:100px;"><asp:Literal runat="server" ID="ltrl_Code" Text='<%# Eval("Code") %>' /></div></ItemTemplate></asp:TemplateField>
            <asp:TemplateField HeaderText="Subject" ><ItemTemplate><div style="min-width:150px;"><asp:Literal runat="server" ID="ltrl_Subject" Text='<%# Eval("Subject") %>' /></div></ItemTemplate></asp:TemplateField>
            <asp:TemplateField HeaderText="Work&nbsp;Description" ><ItemTemplate><div style="min-width:150px;"><asp:Literal runat="server" ID="ltrl_WorkDescription" Text='<%# Eval("WorkDescription") %>' /></div></ItemTemplate></asp:TemplateField>
            <asp:TemplateField HeaderText="Area" ><ItemTemplate><div style="min-width:100px;"><asp:Literal runat="server" ID="ltrl_Area" Text='<%# Eval("Area") %>' /></div></ItemTemplate></asp:TemplateField>            
            <asp:TemplateField HeaderText="Location" ><ItemTemplate><div style="min-width:150px;"><asp:Literal runat="server" ID="ltrl_Location" Text='<%# Eval("Location") %>' /></div></ItemTemplate></asp:TemplateField>
            <asp:TemplateField HeaderText="Category" ><ItemTemplate><div style="min-width:100px;"><asp:Literal runat="server" ID="ltrl_Category" Text='<%# Eval("Category") %>' /></div></ItemTemplate></asp:TemplateField>
            ...
            <asp:TemplateField HeaderText="Period" ><ItemTemplate><asp:Literal runat="server" ID="ltrl_Period" Text='<%# $"{Eval("No")}" == "-1" || $"{Eval("PeriodType")}" == "" ? "" : $"{Eval("Period")} {Eval("PeriodType")}" %>' /></ItemTemplate></asp:TemplateField>
...
<f:Window ID="wSort" Title="Sorting" Hidden="true" EnableIFrame="true" runat="server" EnableMaximize="true" EnableResize="true" Target="Top" IsModal="false" Width="562px" Height="470px" OnClose="wSort_Close"></f:Window>                  
  ...
  <script>       
        function getEditWindowUrl(id) {
            return F.baseUrl + 'Pages/WorkOrder/WorkOrderInput.aspx?Id=' + id + '&parenttabid=' + parent.getActiveTabId();
        }
        function getStockOutWindowUrl(id) {
          return F.baseUrl + 'Pages/Inventory/StockOut.aspx?WorkOrderId=' + id + '&parenttabid=' + parent.getActiveTabId();
        }
