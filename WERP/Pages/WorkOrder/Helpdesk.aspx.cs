...
public partial class Pages_WorkOrder_HelpDesk : PageBase
{
    #region Fields 
    ...
    protected void lb_Click(object sender, EventArgs e)
    {
        ...
        {
            case "lbEdit":
                U.OpenNewTab("EditHelpdesk", "Edit Helpdesk", $"getEditWindowUrl('{lb.CommandArgument}')");
                break;
            case "lbOrder":
    ...
    private void SetInitAdd()
    {
        ...
        joBuilder.AddProperty("id", "NewHelpdesk");
        joBuilder.AddProperty("title", "New Helpdesk");
    ...
    {
        oList.Add(new Helpdesk() { No = -1, WorkOrderType = CNT.DataNotAvailable });
    ...
    private void ExportExcel()
    {
        ...
        List<object> oList = Helpdesk.GetByCriteria(pList, out double TotalRow);
        Wrapping w = new Wrapping { Sheet = sheet, CellStyle = HeaderStyle };

        #region HeaderExcel	
        w.SetExcel("No");
        w.SetExcel("Code");
        w.SetExcel("Work Order Type");
        w.SetExcel("Helpdek Category");        
        w.SetExcel("Subject");
        w.SetExcel("Area");
        w.SetExcel("Location");
        w.SetExcel("Request Type");
        w.SetExcel("Request Source");
        w.SetExcel("Requester Name");
        w.SetExcel("Requester Email");
        w.SetExcel("Requester Phone");
        w.SetExcel("Requester Sosmed");
        w.SetExcel("Request Detail");
        w.SetExcel("Requested");
        w.SetExcel("Cust No");
        w.SetExcel("Cust Name");
        w.SetExcel("Cust Addres");
        w.SetExcel("Response Status");
        w.SetExcel("Status");
        w.SetExcel("Completion");
        w.SetExcel("Target Completion Status");
        #endregion

        #region DataExcel		
        w.Column = 0;
        w.CellStyle = ItemStyleOdd;
        foreach (Helpdesk o in oList)
        {
            w.Row += 1;
            w.SetExcel(o.No, true);
            w.SetExcel(o.Code);
            w.SetExcel(o.WorkOrderType);
            w.SetExcel(o.HelpdeskCategory);
            w.SetExcel(o.Subject);
            w.SetExcel(o.Area);
            w.SetExcel(o.Location);
            w.SetExcel(o.RequestType);
            w.SetExcel(o.RequestSource);
            w.SetExcel(o.RequesterName);
            w.SetExcel(o.RequesterEmail);
            w.SetExcel(o.RequesterPhone);
            w.SetExcel(o.SocialMedia);
            w.SetExcel(o.RequestDetail);
            w.SetExcel(o.Requested);
            w.SetExcel(o.CustNo);
            w.SetExcel(o.CustName);
            w.SetExcel(o.CustAddress);
            w.SetExcel(o.ResponseStatus);
            w.SetExcel(o.Status);
            w.SetExcel(o.Completion);
            w.SetExcel(o.TargetCompletionStatus);
            w.Column = 0;
        }
        #endregion
