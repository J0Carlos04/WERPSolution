<%@ Page Language="C#" AutoEventWireup="true" CodeFile="MaintenanceScheduler.aspx.cs" Inherits="Pages_WorkOrder_ScheduleMaintenance" %>
      ...
      <Content><div class="wrapper-suppress">                  
        <asp:GridView runat="server" ID="gvData" AutoGenerateColumns="false" CssClass="table table-striped table-bordered table-hover" OnRowDataBound="gvData_RowDataBound" Width="100%" HeaderStyle-HorizontalAlign="Center">
          <Columns>
            ...
            <asp:TemplateField HeaderText="Subject" ><ItemTemplate><asp:Literal runat="server" ID="ltrl_Subject" Text='<%# Eval("Subject") %>' /></ItemTemplate></asp:TemplateField>
            <asp:TemplateField HeaderText="Work&nbsp;Description" ><ItemTemplate><asp:Literal runat="server" ID="ltrl_WorkDescription" Text='<%# Eval("WorkDescription") %>' /></ItemTemplate></asp:TemplateField>
            <asp:TemplateField HeaderText="Area" ><ItemTemplate><asp:Literal runat="server" ID="ltrl_Area" Text='<%# Eval("Area") %>' /></ItemTemplate></asp:TemplateField>            
            <asp:TemplateField HeaderText="Location" ><ItemTemplate><asp:Literal runat="server" ID="ltrl_Location" Text='<%# Eval("Location") %>' /></ItemTemplate></asp:TemplateField>
            <asp:TemplateField HeaderText="Work&nbsp;Order&nbsp;Category" ><ItemTemplate><asp:Literal runat="server" ID="ltrl_WorkOrderCategory" Text='<%# Eval("WorkOrderCategory") %>' /></ItemTemplate></asp:TemplateField>
            ...
            <asp:TemplateField HeaderText="EndDate" ><ItemTemplate><asp:Literal runat="server" ID="ltrl_EndDate" Text='<%# $"{Eval("EndDate")}" == "1/1/0001 12:00:00 AM" ? "" : $"{Eval("EndDate", "{0: dd-MMM-yyyy}")}" %>' /></ItemTemplate></asp:TemplateField>                                    
        </Columns>
...
<f:Window ID="wSort" Title="Sorting" Hidden="true" EnableIFrame="true" runat="server" EnableMaximize="true" EnableResize="true" Target="Top" IsModal="false" Width="562px" Height="470px" OnClose="wSort_Close"></f:Window>                  
  ...
  <script>
        function getNewWindowUrl() {
            return F.baseUrl + 'Pages/WorkOrder/MaintenanceSchedulerInput.aspx?parenttabid=' + parent.getActiveTabId();
        }
        function getEditWindowUrl(id) {
            return F.baseUrl + 'Pages/WorkOrder/MaintenanceSchedulerInput.aspx?Id=' + id + '&parenttabid=' + parent.getActiveTabId();
        }
        function getOrderWindowUrl(id) {
          return F.baseUrl + 'Pages/WorkOrder/WorkOrder.aspx?ScheduleMaintenanceId=' + id + '&parenttabid=' + parent.getActiveTabId();
        }
