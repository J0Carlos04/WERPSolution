<%@ Page Language="C#" AutoEventWireup="true" CodeFile="DataLogger.aspx.cs" Inherits="Pages_Logger_DataLogger" %>
              ...
              <asp:GridView runat="server" ID="gvData" AutoGenerateColumns="false" CssClass="table table-striped table-bordered table-hover" OnRowDataBound="gvData_RowDataBound" Width="100%" HeaderStyle-HorizontalAlign="Center">
                <Columns>
                  ...
                  <asp:TemplateField HeaderText="Station">
                    <ItemTemplate><asp:Literal runat="server" ID="ltrlStation" Text='<%# Eval("Station").Is(CNT.DataNotAvailable) ? "" :  Eval("Station") %>' Visible='<%# Eval("Mode").IsEmpty() %>' /></ItemTemplate>
                  </asp:TemplateField>
                  <asp:TemplateField HeaderText="Function Logger"><ItemTemplate><asp:Literal runat="server" ID="ltrlFunctionLogger" Text='<%# Eval("FunctionLogger") %>' /></ItemTemplate></asp:TemplateField>                  
                  <asp:TemplateField HeaderText="Date"><ItemTemplate><asp:Literal runat="server" ID="ltrldate" Text='<%# Eval("date") %>' /></ItemTemplate></asp:TemplateField>
                  <asp:TemplateField HeaderText="Value"><ItemTemplate><asp:Literal runat="server" ID="ltrlvalue" Text='<%# Eval("value") %>' /></ItemTemplate></asp:TemplateField>
                </Columns>
