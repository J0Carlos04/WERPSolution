<%@ Page Language="C#" AutoEventWireup="true" CodeFile="MappingTotalizer.aspx.cs" Inherits="Pages_Logger_MappingTotalizer" %>
...
<f:Panel ID="pContent" runat="server" ShowBorder="false" ShowHeader="false" Layout="Region" EnableAjaxLoading="true" AjaxLoadingType="Default">
  <Toolbars>
    <f:Toolbar runat="server" ID="tTop" Position="Top"><Items>
      ...
      <f:Button runat="server" ID="btnDelete" Text="Delete" Icon="Delete" OnClick="btn_Click" ConfirmText="Are you sure you want to delete this Mapping Totalizer?. This action cannot be undone. Do you wish to proceed?" ConfirmIcon="Warning" />
      ...
      <Content>                  
        <asp:GridView runat="server" ID="gvData" AutoGenerateColumns="false" CssClass="table table-striped table-bordered table-hover" OnRowDataBound="gvData_RowDataBound" Width="100%" HeaderStyle-HorizontalAlign="Center">
          <Columns>
            ...
            <asp:TemplateField HeaderText="Totalizer" >              
              <ItemTemplate>		    
                <asp:Literal runat="server" ID="ltrl_Name" Text='<%# Eval("Name") %>' Visible='<%# $"{Eval("Mode")}" == "" ? true : false %>' />                        
                <asp:DropDownList runat="server" ID="ddl_Id" Visible='<%# $"{Eval("Mode")}" == "" ? false : true %>' CssClass="form-control form-control-sm" AutoPostBack="true" OnSelectedIndexChanged="ddl_SelectedIndexChanged" />
              </ItemTemplate>
            </asp:TemplateField>            
            <asp:TemplateField HeaderText="Parent Totalizer" >              
              <ItemTemplate>		    
                <asp:Literal runat="server" ID="ltrl_Parent" Text='<%# Eval("Parent") %>' Visible='<%# $"{Eval("Mode")}" == "" ? true : false %>' />                        
                <asp:DropDownList runat="server" ID="ddl_ParentId" Visible='<%# $"{Eval("Mode")}" == "" ? false : true %>' CssClass="form-control form-control-sm" />
              </ItemTemplate>
            </asp:TemplateField> 
        </Columns>        
        </asp:GridView>      
