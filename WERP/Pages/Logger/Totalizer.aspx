<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Totalizer.aspx.cs" Inherits="Pages_Logger_Totalizer" %>
...
<f:Panel ID="pContent" runat="server" ShowBorder="false" ShowHeader="false" Layout="Region" EnableAjaxLoading="true" AjaxLoadingType="Default">
  <Toolbars>
    <f:Toolbar runat="server" ID="tTop" Position="Top"><Items>
      ...
      <f:Button runat="server" ID="btnDelete" Text="Delete" Icon="Delete" OnClick="btn_Click" ConfirmText="Are you sure you want to delete this Totalizer?. This action cannot be undone. Do you wish to proceed?" ConfirmIcon="Warning" />
        ...
        <asp:GridView runat="server" ID="gvData" AutoGenerateColumns="false" CssClass="table table-striped table-bordered table-hover" OnRowDataBound="gvData_RowDataBound" Width="100%" HeaderStyle-HorizontalAlign="Center">
          <Columns>
            ...
            <asp:TemplateField HeaderText="Function&nbsp;Logger" >              
              <ItemTemplate>		    
                <asp:Literal runat="server" ID="ltrl_FunctionLogger" Text='<%# Eval("FunctionLogger") %>' Visible='<%# $"{Eval("Mode")}" == "" ? true : false %>' />                        
                <asp:DropDownList runat="server" ID="ddl_FunctionLoggerId" Visible='<%# $"{Eval("Mode")}" == "" ? false : true %>' CssClass="form-control form-control-sm" />
              </ItemTemplate>
            </asp:TemplateField> 
            <asp:TemplateField HeaderText="Parent" ><ItemTemplate><asp:Literal runat="server" ID="ltrl_Parent" Text='<%# Eval("Parent") %>' /></ItemTemplate></asp:TemplateField>         
