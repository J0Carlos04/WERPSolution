...
public partial class Pages_Logger_Function : System.Web.UI.Page
{
    #region Handlers
    ...
    protected void gvData_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        else if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            Function o = (Function)oList[e.Row.RowIndex];
            cb_IsChecked.Enabled = oList.Exists(a => $"{((Function)a).Mode}" != "") ? false : true;
    ...
    protected void ucPaging_OpChanged(object sender, EventArgs e)
    {
        ...
        List<object> oList = oList = Function.GetByCriteria(pList, out double TotalRow);
    ...
    protected void wSearch_Close(object sender, F.WindowCloseEventArgs e)
    {
        ...
        btnSearch.OnClientClick = wSearch.GetShowReference($"~/Pages/Search.aspx?ClassName=Function&SearchData={arrArgument[1].Replace(Environment.NewLine, "~")}");
    ...
    protected void wSort_Close(object sender, F.WindowCloseEventArgs e)
    {
        ...
        btnSorting.OnClientClick = wSort.GetShowReference($"~/Pages/Sort.aspx?ClassName=Function&SortData={arrArgument[1].Replace(Environment.NewLine, "~")}");
    ...
    private void SetInit()
    {
        ...
        btnSearch.OnClientClick = $"{wSearch.GetShowReference($"~/Pages/Search.aspx?ClassName=Function")}return false;";
        btnSorting.OnClientClick = $"{wSort.GetShowReference($"~/Pages/Sort.aspx?ClassName=Function")}return false;";
        ViewState[CNT.VS.Table] = "Function";
    }
    ...
    {        
        ...
        List<object> oList = Function.GetByCriteria(pList, out TotalRow);
    ...
    {
        oList.Add(new Function() { No = -1, Name = CNT.DataNotAvailable });
    ...
    private void SetInitAdd()
    {
        ...
        List<object> oList = U.GetGridData(gvData, typeof(Function)).ListData.FindAll(a => ((Function)a).Name != CNT.DataNotAvailable);
        oList.ForEach(a => { ((Function)a).IsChecked = false; ((Function)a).Mode = ""; });
        oList.Insert(0, new Function() { No = 0, IsChecked = true, Mode = "add" });
        U.BindGrid(gvData, oList);
    ...
    {
        List<object> oList = U.GetGridData(gvData, typeof(Function)).ListData;
        if (!oList.Exists(a => ((Function)a).IsChecked))
        {
        ...
        oList.ForEach(a => { ((Function)a).Mode = ((Function)a).IsChecked ? "edit" : ""; });
        U.BindGrid(gvData, oList);
        ((CheckBox)gvData.HeaderRow.Cells[0].FindControl("cbCheckAll")).Enabled = false;
    }
    private void Cancel()
    ...
    private void SaveAdd()
    {
        try
        {
            ...
            Function oTemp = Function.GetByKey(o.Name, o.Id);
            if (oTemp.Id != 0)
            {
    ...
    private void SaveEdit()
    {
        ...
        foreach (Function o in oList)
        {
            ...
            if (oTemp.Id != 0)
            {
                U.ShowMessage($"Save Failed, {Environment.NewLine}Function with Name {o.Name} already exist");
                return;
            }
    ...
    private void Delete()
    {
        try
        {
            string ErrorMessage = "";
            List<object> oList = U.GetGridData(gvData, typeof(Function)).ListData;
            if (oList.FindAll(a => ((Function)a).IsChecked).Count == 0)
            {
            ...
            {
                Function o = (Function)oList[i];
                if (!o.IsChecked) continue;

                ErrorMessage = DataAccess.Delete(0, o.Id, "Id", "Function");
    ...
    private void ExportExcel()
    {
        ...
        List<object> oList = Function.GetByCriteria(pList, out double TotalRow);
        ...
        resp.Clear();
        resp.Buffer = false;
        resp.AppendHeader("content-disposition", string.Format("attachment; filename=Function.xlsx"));
        ...
        byte[] buffer = null;
        string tempFilename = $@"{U.PathTempFolder}Function.xlsx";
