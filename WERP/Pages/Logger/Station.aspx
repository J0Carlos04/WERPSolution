<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Station.aspx.cs" Inherits="Pages_Logger_Station" %>
    ...
    <f:Panel ID="pContent" runat="server" ShowBorder="false" ShowHeader="false" Layout="Region" EnableAjaxLoading="true" AjaxLoadingType="Default">
      <Toolbars>
        <f:Toolbar runat="server" ID="tTop" Position="Top"><Items>
          ...
          <f:Button runat="server" ID="btnDelete" Text="Delete" Icon="Delete" OnClick="btn_Click" ConfirmText="Are you sure you want to delete this Station?. This action cannot be undone. Do you wish to proceed?" ConfirmIcon="Warning" />
            ...
            <div class="wrapper-suppress">
              <asp:GridView runat="server" ID="gvData" AutoGenerateColumns="false" CssClass="table table-striped table-bordered table-hover" OnRowDataBound="gvData_RowDataBound" Width="100%" HeaderStyle-HorizontalAlign="Center">
                <Columns>
                  ...
                  <asp:TemplateField HeaderText="SiteNumber">
                    <ItemTemplate>
                      <asp:Literal runat="server" ID="ltrlSiteNumber" Text='<%# Eval("Name").Is(CNT.DataNotAvailable) ? "" :  Eval("SiteNumber") %>' Visible='<%# Eval("Mode").IsEmpty() %>' />
                      <asp:TextBox runat="server" ID="tb_SiteNumber" Text='<%# $"{Eval("Mode")}" == "add" ? "" : $"{Eval("SiteNumber")}" %>' Visible='<%# !Eval("Mode").IsEmpty() %>' CssClass="form-control form-control-sm" TextMode="Number" />
                    </ItemTemplate>
                  </asp:TemplateField>
