<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Function.aspx.cs" Inherits="Pages_Logger_Function" %>
    ...
    <f:Panel ID="pContent" runat="server" ShowBorder="false" ShowHeader="false" Layout="Region" EnableAjaxLoading="true" AjaxLoadingType="Default">
      <Toolbars>
        <f:Toolbar runat="server" ID="tTop" Position="Top"><Items>
          ...
          <f:Button runat="server" ID="btnDelete" Text="Delete" Icon="Delete" OnClick="btn_Click" ConfirmText="Are you sure you want to delete this Function?. This action cannot be undone. Do you wish to proceed?" ConfirmIcon="Warning" />
            ...
            <asp:GridView runat="server" ID="gvData" AutoGenerateColumns="false" CssClass="table table-striped table-bordered table-hover" OnRowDataBound="gvData_RowDataBound" Width="100%" HeaderStyle-HorizontalAlign="Center">
              <Columns>
                ...
                <asp:TemplateField HeaderText="Name">
                  <ItemTemplate>
                    <asp:Literal runat="server" ID="ltrlName" Text='<%# Eval("Name") %>' Visible='<%# $"{Eval("Mode")}" == "" ? true : false %>' />
                    <asp:TextBox runat="server" ID="tb_Name" Text='<%# $"{Eval("Mode")}" == "add" ? "" : $"{Eval("Name")}" %>' Visible='<%# $"{Eval("Mode")}" == "" ? false : true %>' CssClass="form-control form-control-sm" />
                  </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="Level">
                  <ItemTemplate>
                    <asp:Literal runat="server" ID="ltrlLevel" Text='<%# $"{Eval("Name")}" == CNT.DataNotAvailable ? "" : $"{Eval("Level")}" %>' Visible='<%# $"{Eval("Mode")}" == "" ? true : false %>' />
                    <asp:TextBox runat="server" ID="tb_Level" Text='<%# $"{Eval("Mode")}" == "add" ? "" : $"{Eval("Level")}" %>' TextMode="Number" Visible='<%# $"{Eval("Mode")}" == "" ? false : true %>' CssClass="form-control form-control-sm" />
                  </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="UseLocation">
                  <ItemTemplate>
                    <asp:Literal runat="server" ID="ltrlUseLocation" Text='<%# $"{Eval("No")}" == "-1" ? "" : $"{Eval("UseLocation")}" %>' Visible='<%# $"{Eval("Mode")}" == "" ? true : false %>' />
                    <asp:CheckBox runat="server" ID="cb_UseLocation" Checked='<%# Eval("UseLocation") %>' Visible='<%# Eval("Mode").IsNotEmpty() %>' />
                  </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="UseCustomer">
                <ItemTemplate>
                  <asp:Literal runat="server" ID="ltrlUseCustomer" Text='<%# $"{Eval("No")}" == "-1" ? "" : $"{Eval("UseCustomer")}" %>' Visible='<%# $"{Eval("Mode")}" == "" ? true : false %>' />
                  <asp:CheckBox runat="server" ID="cb_UseCustomer" Checked='<%# Eval("UseCustomer") %>' Visible='<%# Eval("Mode").IsNotEmpty() %>' />
                </ItemTemplate>
              </asp:TemplateField>
                <asp:TemplateField HeaderText="Active">
                  <ItemTemplate>
                    <asp:Literal runat="server" ID="ltrlActive" Text='<%# $"{Eval("No")}" == "-1" ? "" : $"{Eval("Active")}" %>' Visible='<%# $"{Eval("Mode")}" == "" ? true : false %>' />
                    <asp:DropDownList runat="server" ID="ddl_Active" Visible='<%# $"{Eval("No")}" == "0" || $"{Eval("Mode")}" == "" ? false : true %>' CssClass="form-select form-select-sm">
