...
public partial class Pages_Logger_DataLogger : System.Web.UI.Page
{
    #region Handlers
    ...
    protected void gvData_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        else if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            //DataLogger o = (DataLogger)oList[e.Row.RowIndex];
            //cb_IsChecked.Enabled = oList.Exists(a => $"{((DataLogger)a).Mode}" != "") ? false : true;            
        }
    }
    protected void ucPaging_OpChanged(object sender, EventArgs e)
    {
        #region GetCiteria
        List<clsParameter> pList = U.GetParameter(ViewState[CNT.VS.Criteria].ToText().Replace("Where", ""), ViewState[CNT.VS.SortBy]);
        ...
        List<object> oList = oList = DataLogger.GetByCriteria(pList, out double TotalRow);
    ...
    protected void wSearch_Close(object sender, F.WindowCloseEventArgs e)
    {
        ...
        btnSearch.OnClientClick = wSearch.GetShowReference($"~/Pages/Search.aspx?ClassName=DataLogger&SearchData={arrArgument[1].Replace(Environment.NewLine, "~")}");
    ...
    protected void wSort_Close(object sender, F.WindowCloseEventArgs e)
    {
        ...
        btnSorting.OnClientClick = wSort.GetShowReference($"~/Pages/Sort.aspx?ClassName=DataLogger&SortData={arrArgument[1].Replace(Environment.NewLine, "~")}");
    ...
    private void SetInit()
    {
        ...
        btnSearch.OnClientClick = $"{wSearch.GetShowReference($"~/Pages/Search.aspx?ClassName=DataLogger")}return false;";
        btnSorting.OnClientClick = $"{wSort.GetShowReference($"~/Pages/Sort.aspx?ClassName=DataLogger")}return false;";
        ViewState[CNT.VS.Table] = "DataLogger";
    }
    ...
    private void BindData()
    {
        ...
        List<object> oList = DataLogger.GetByCriteria(pList, out TotalRow);
    ...
    {
        oList.Add(new DataLogger() { No = -1, Station = CNT.DataNotAvailable });        
    ...
    private void ExportExcel()
    {
        ...
        List<object> oList = DataLogger.GetByCriteria(pList, out double TotalRow);
        ...
        resp.Clear();
        resp.Buffer = false;
        resp.AppendHeader("content-disposition", string.Format("attachment; filename=DataLogger.xlsx"));
        ...
        byte[] buffer = null;
        string tempFilename = $@"{U.PathTempFolder}DataLogger.xlsx";
