...
public partial class Pages_Logger_FunctionLogger  : PageBase
{    
    #region Handlers
    ...
    protected void gvData_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        else if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            DropDownList ddl_UnitId = (DropDownList)e.Row.FindControl("ddl_UnitId");
            ...
            FunctionLogger o = (FunctionLogger)oList[e.Row.RowIndex];
            U.SetDropDownMasterData(ddl_UnitId, "Unit");
            U.SetDropDownMasterData(ddl_StationId, "Station");
            ddl_UnitId.SetValue(o.UnitId);
            ddl_StationId.SetValue(o.StationId);
            cb_IsChecked.Enabled = oList.Exists(a => $"{((FunctionLogger)a).Mode}" != "") ? false : true;            
    ...
    protected void ucPaging_OpChanged(object sender, EventArgs e)
    {
        ...
        List<object> oList = oList = FunctionLogger.GetByCriteria(pList, out double TotalRow);
    ...
    protected void wSearch_Close(object sender, F.WindowCloseEventArgs e)
    {
        ...
        btnSearch.OnClientClick = wSearch.GetShowReference($"~/Pages/Search.aspx?ClassName=FunctionLogger&SearchData={arrArgument[1].Replace(Environment.NewLine, "~")}");
    ...
    protected void wSort_Close(object sender, F.WindowCloseEventArgs e)
    {
        ...
        btnSorting.OnClientClick = wSort.GetShowReference($"~/Pages/Sort.aspx?ClassName=FunctionLogger&SortData={arrArgument[1].Replace(Environment.NewLine, "~")}");
    ...
    private void SetInit()
    {
        ...
        btnSearch.OnClientClick = $"{wSearch.GetShowReference($"~/Pages/Search.aspx?ClassName=FunctionLogger")}return false;";
        btnSorting.OnClientClick = $"{wSort.GetShowReference($"~/Pages/Sort.aspx?ClassName=FunctionLogger")}return false;";        
    }
    ...
    private void BindData()
    {
        ...
        List<object> oList = FunctionLogger.GetByCriteria(pList, out TotalRow);
    ...
    {
        oList.Add(new FunctionLogger() { No = -1, Name = CNT.DataNotAvailable });
    ...
    private void SetInitAdd()
    {
        ...
        List<object> oList = U.GetGridData(gvData, typeof(FunctionLogger)).ListData.FindAll(a => ((FunctionLogger)a).Name != CNT.DataNotAvailable);
        oList.ForEach(a => { ((FunctionLogger)a).IsChecked = false; ((FunctionLogger)a).Mode = ""; });        
        oList.Insert(0, new FunctionLogger() { No = 0, IsChecked = true, Mode = "add" });
        U.BindGrid(gvData, oList);
    ...
    {
        List<object> oList = U.GetGridData(gvData, typeof(FunctionLogger)).ListData;
        if (!oList.Exists(a => ((FunctionLogger)a).IsChecked))
        {
        ...
        oList.ForEach(a => { ((FunctionLogger)a).Mode = ((FunctionLogger)a).IsChecked ? "edit" : ""; });
        U.BindGrid(gvData, oList);
        ((CheckBox)gvData.HeaderRow.Cells[0].FindControl("cbCheckAll")).Enabled = false;        
    }
    private void Cancel()
    ...
    private void SaveAdd()
    {
        try
        {
            string Result = "";
            List<object> oList = U.GetGridData(gvData, typeof(FunctionLogger)).ListData;
            FunctionLogger o = (FunctionLogger)oList[0];
            ...
            FunctionLogger oTemp = FunctionLogger.GetByKey(o.Name, o.Id);
            if (oTemp.Id != 0)
            {
    ...
    private void SaveEdit()
    {
        ...
        foreach (FunctionLogger o in oList)
        {
            ...
            if (oTemp.Id != 0)
            {
                U.ShowMessage($"Save Failed, {Environment.NewLine}FunctionLogger with Name {o.Name} already exist");
                return;
            }
    ...
    private void Delete()
    {
        try
        {
            string ErrorMessage = "";
            List<object> oList = U.GetGridData(gvData, typeof(FunctionLogger)).ListData;
            if (oList.FindAll(a => ((FunctionLogger)a).IsChecked).Count == 0)
            {
            ...
            {
                FunctionLogger o = (FunctionLogger)oList[i];
                if (!o.IsChecked) continue;

                ErrorMessage = DataAccess.Delete(0, o.Id, "Id", "FunctionLogger");
    ...
    private void ExportExcel()
    {
        ...
        List<object> oList = FunctionLogger.GetByCriteria(pList, out double TotalRow);
        ...
        resp.Clear();
        resp.Buffer = false;
        resp.AppendHeader("content-disposition", string.Format("attachment; filename=FunctionLogger.xlsx"));
        ...
        byte[] buffer = null;
        string tempFilename = $@"{U.PathTempFolder}FunctionLogger.xlsx";
