...
public partial class Pages_Logger_MappingTotalizer : PageBase
{    
    #region Handlers
    ...
    protected void gvData_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        else if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            DropDownList ddl_Id = (DropDownList)e.Row.FindControl("ddl_Id");
            DropDownList ddl_ParentId = (DropDownList)e.Row.FindControl("ddl_ParentId");

            List<object> oList = (List<object>)gvData.DataSource;
            Totalizer o = (Totalizer)oList[e.Row.RowIndex];

            U.SetTotalizer(ddl_Id);
            ddl_Id.SetValue(o.Id);
            U.SetParentTotalizer(ddl_ParentId, ddl_Id.SelectedValue);
            ddl_ParentId.SetValue(o.ParentId);
            cb_IsChecked.Enabled = oList.Exists(a => $"{((Totalizer)a).Mode}" != "") ? false : true; 
            
            if (o.Mode == "add")
                U.SetNoParentTotalizer(ddl_Id);
        }
    }
    protected void ucPaging_OpChanged(object sender, EventArgs e)
    {
        ...
        List<object> oList = oList = Totalizer.ParentGetByCriteria(pList, out double TotalRow);
    ...
    private void TotalizerChanged(DropDownList ddl)
    {
        DropDownList ddl_ParentId = (DropDownList)ddl.FindControl("ddl_ParentId");
        ddl_ParentId.Items.Clear();
        if (ddl.SelectedValue.IsEmpty()) return;
        U.SetParentTotalizer(ddl_ParentId, ddl.SelectedValue);
    }
    ...
    private object SetCriteria(object Criteria)
    {
        ...
        Criteria = $"{Criteria} (a.ParentId is not null) ";
        return Criteria;
    }
    private void BindData()
    {
        ...
        List<object> oList = Totalizer.ParentGetByCriteria(pList, out TotalRow);
    ...
    private void SaveAdd()
    {
        try
        {
            ...
            #region Validation	
            if (o.Id == 0 || o.ParentId == 0)
            {
                U.ShowMessage($"Save Failed, {Environment.NewLine}Totalizer & Parent Totalizer is required");
                return;
            }
            StringBuilder sb = new StringBuilder();
            Totalizer oTemp = Totalizer.GetById(o.Id);            
            if (oTemp.ParentId != 0) 
                sb.AppendLine($"1. Totalizer {oTemp.Name} already mapped");
            Totalizer oTempParent = Totalizer.GetByParentId(o.ParentId);            
            if (!sb.IsEmpty())
            {
                U.ShowMessage($"Save Failed, {Environment.NewLine}{sb}");
                return;
            }                        
            #endregion
                      
            Result = o.UpdateParentId(o.Id, o.ParentId, U.GetUsername());
    ...
    {
        string Result = "";
        List<object> oList = U.GetGridData(gvData, typeof(Totalizer)).ListData.FindAll(a => ((Totalizer)a).IsChecked);
        StringBuilder sb = new StringBuilder();
        int Line = 0;
        int Seq = 0;
        foreach (Totalizer o in oList)
        {
            ...
            {
                Seq += 1;
                sb.AppendLine($"{Seq}. Totalizer & Parent Totalizer at line {Line} is required");
            }            
        }
        if (!sb.IsEmpty())
        {
    ...
    private void Delete()
    {
        try
        {
            ...
            for (int i = 0; i < oList.Count; i++)
            {
                ...
                Result = o.DeleteMapping(o.Id, U.GetUsername());
                if (Result.Contains("Error Message"))
                {
                    U.ShowMessage($"Delete Failed,{Environment.NewLine} {Result}");
                    return;
                }
                oList.Remove(o);
                i -= 1;
    ...
    private void ExportExcel()
    {
        ...
        List<object> oList = Totalizer.ParentGetByCriteria(pList, out double TotalRow);
        Wrapping w = new Wrapping { Sheet = sheet, CellStyle = HeaderStyle };

        #region HeaderExcel	
        w.SetExcel("No");
        w.SetExcel("Totalizer");
        w.SetExcel("Parent Totalizer");
        w.SetExcel("Active");
        #endregion
