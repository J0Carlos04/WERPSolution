...
public partial class Pages_Logger_FunctionStation : PageBase
{
    #region Handlers
    ...
    protected void gvData_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        else if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            DropDownList ddl_FunctionId = (DropDownList)e.Row.FindControl("ddl_FunctionId");
            DropDownList ddl_StationId = (DropDownList)e.Row.FindControl("ddl_StationId");

            List<object> oList = (List<object>)gvData.DataSource;
            FunctionStation o = (FunctionStation)oList[e.Row.RowIndex];
            cb_IsChecked.Enabled = oList.Exists(a => $"{((FunctionStation)a).Mode}" != "") ? false : true;
            U.SetDropDownMasterData(ddl_FunctionId, "[Function]");
            U.SetDropDownMasterData(ddl_StationId, "[Station]");
            ddl_FunctionId.SetValue(o.FunctionId);
    ...
    protected void ucPaging_OpChanged(object sender, EventArgs e)
    {
        ...
        List<object> oList = oList = FunctionStation.GetByCriteria(pList, out double TotalRow);
    ...
    protected void wSearch_Close(object sender, F.WindowCloseEventArgs e)
    {
        ...
        btnSearch.OnClientClick = wSearch.GetShowReference($"~/Pages/Search.aspx?ClassName=FunctionStation&SearchData={arrArgument[1].Replace(Environment.NewLine, "~")}");
    ...
    protected void wSort_Close(object sender, F.WindowCloseEventArgs e)
    {
        ...
        btnSorting.OnClientClick = wSort.GetShowReference($"~/Pages/Sort.aspx?ClassName=FunctionStation&SortData={arrArgument[1].Replace(Environment.NewLine, "~")}");
    ...
    private void SetInit()
    {
        ...
        btnSearch.OnClientClick = $"{wSearch.GetShowReference($"~/Pages/Search.aspx?ClassName=FunctionStation")}return false;";
        btnSorting.OnClientClick = $"{wSort.GetShowReference($"~/Pages/Sort.aspx?ClassName=FunctionStation")}return false;";
        ViewState[CNT.VS.Table] = "FunctionStation";
    }
    ...
    private void BindData()
    {
        ...
        List<object> oList = FunctionStation.GetByCriteria(pList, out TotalRow);
    ...
    {
        oList.Add(new FunctionStation() { No = -1, Function = CNT.DataNotAvailable });
    ...
    private void SetInitAdd()
    {
        ...
        List<object> oList = U.GetGridData(gvData, typeof(FunctionStation)).ListData.FindAll(a => ((FunctionStation)a).Function != CNT.DataNotAvailable);
        oList.ForEach(a => { ((FunctionStation)a).IsChecked = false; ((FunctionStation)a).Mode = ""; });
        oList.Insert(0, new FunctionStation() { No = 0, IsChecked = true, Mode = "add" });
        U.BindGrid(gvData, oList);
    ...
    {
        List<object> oList = U.GetGridData(gvData, typeof(FunctionStation)).ListData;
        if (!oList.Exists(a => ((FunctionStation)a).IsChecked))
        {
        ...
        oList.ForEach(a => { ((FunctionStation)a).Mode = ((FunctionStation)a).IsChecked ? "edit" : ""; });
        U.BindGrid(gvData, oList);
        ((CheckBox)gvData.HeaderRow.Cells[0].FindControl("cbCheckAll")).Enabled = false;
    }
    private void Cancel()
    ...
    private void SaveAdd()
    {
        try
        {
            string Result = "";
            List<object> oList = U.GetGridData(gvData, typeof(FunctionStation)).ListData;
            FunctionStation o = (FunctionStation)oList[0];
            ...
            FunctionStation oTemp = FunctionStation.GetByKey(o.FunctionId, o.StationId);
            if (oTemp.Id != 0)
            {
                U.ShowMessage($"Save Failed, {Environment.NewLine}Function with Name {o.Function} and Station with Name {o.Station} already exist");
                return;
            }
            #endregion
    ...
    private void SaveEdit()
    {
        ...
        foreach (FunctionStation o in oList)
        {
            if (!o.IsChecked) continue;
            FunctionStation oTemp = FunctionStation.GetByKey(o.FunctionId, o.StationId, o.Id);
            if (oTemp.Id != 0)
            {
                U.ShowMessage($"Save Failed, {Environment.NewLine}Function with Name {o.Function} and Station with Name {o.Station} already exist");
                return;
    ...
    private void Delete()
    {
        try
        {
            string ErrorMessage = "";
            List<object> oList = U.GetGridData(gvData, typeof(FunctionStation)).ListData;
            if (oList.FindAll(a => ((FunctionStation)a).IsChecked).Count == 0)
            {
            ...
            {
                FunctionStation o = (FunctionStation)oList[i];
                if (!o.IsChecked) continue;

                ErrorMessage = DataAccess.Delete(0, o.Id, "Id", "FunctionStation");
    ...
    private void ExportExcel()
    {
        ...
        List<object> oList = FunctionStation.GetByCriteria(pList, out double TotalRow);
        ...
        resp.Clear();
        resp.Buffer = false;
        resp.AppendHeader("content-disposition", string.Format("attachment; filename=FunctionStation.xlsx"));
        ...
        byte[] buffer = null;
        string tempFilename = $@"{U.PathTempFolder}FunctionStation.xlsx";
