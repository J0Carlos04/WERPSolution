...
public partial class Pages_Logger_FunctionLoggerCustomer : System.Web.UI.Page
{
    #region Handlers
    ...
    protected void gvData_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        else if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            DropDownList ddl_StationId = (DropDownList)e.Row.FindControl("ddl_StationId");
            DropDownList ddl_FunctionLoggerId = (DropDownList)e.Row.FindControl("ddl_FunctionLoggerId");
            Literal ltrl_CustomerId = (Literal)e.Row.FindControl("ltrl_CustomerId");
            TextBox tb_Customer = (TextBox)e.Row.FindControl("tb_Customer");
            Button btnLookupCust = (Button)e.Row.FindControl("btnLookupCust");            
            btnLookupCust.OnClientClick = $"{wCust.GetShowReference($"~/Pages/WorkOrder/SelectCust.aspx?Select=Single&RowIndex={e.Row.RowIndex}")}return false;";

            List<object> oList = (List<object>)gvData.DataSource;
            FunctionLoggerCustomer o = (FunctionLoggerCustomer)oList[e.Row.RowIndex];
            cb_IsChecked.Enabled = oList.Exists(a => $"{((FunctionLoggerCustomer)a).Mode}" != "") ? false : true;
            U.SetDropDownMasterData(ddl_StationId, "[Station]");
            ddl_StationId.SetValue(o.StationId);
            if (o.StationId.IsNotZero())
            {
                U.SetDropDownFunctionLoggerByStation(ddl_FunctionLoggerId, o.StationId);
                ddl_FunctionLoggerId.SetValue(o.FunctionLoggerId);
            }
            if (o.CustomerId.IsNotZero())
            {
    ...
    protected void ucPaging_OpChanged(object sender, EventArgs e)
    {
        ...
        List<object> oList = oList = FunctionLoggerCustomer.GetByCriteria(pList, out double TotalRow);
    ...
    protected void wSearch_Close(object sender, F.WindowCloseEventArgs e)
    {
        ...
        btnSearch.OnClientClick = wSearch.GetShowReference($"~/Pages/Search.aspx?ClassName=FunctionLoggerCustomer&SearchData={arrArgument[1].Replace(Environment.NewLine, "~")}");
    ...
    protected void wSort_Close(object sender, F.WindowCloseEventArgs e)
    {
        ...
        btnSorting.OnClientClick = wSort.GetShowReference($"~/Pages/Sort.aspx?ClassName=FunctionLoggerCustomer&SortData={arrArgument[1].Replace(Environment.NewLine, "~")}");
        btnSorting.OnClientClick += "return false;";
        BindData();
    }
    protected void wCust_Close(object sender, F.WindowCloseEventArgs e)
    {
        string[] arr = e.CloseArgument.Split('|');
        string Id = arr[0];
        string RowIndex = arr[1];
        foreach (GridViewRow Row in gvData.Rows)
        {
            if (RowIndex.Is(Row.RowIndex))
            {
                Literal ltrl_CustomerId = (Literal)Row.FindControl("ltrl_CustomerId");
                TextBox tb_Customer = (TextBox)Row.FindControl("tb_Customer");
                Customer c = BaseModel.GetById<Customer>(Id);
                ltrl_CustomerId.Text = arr[0];
                tb_Customer.Text = $"{c.CustNo} - {c.Name}";
                return;
            }
            else continue;
        }
    ...
    private void SetInit()
    {
        ...
        btnSearch.OnClientClick = $"{wSearch.GetShowReference($"~/Pages/Search.aspx?ClassName=FunctionLoggerCustomer")}return false;";
        btnSorting.OnClientClick = $"{wSort.GetShowReference($"~/Pages/Sort.aspx?ClassName=FunctionLoggerCustomer")}return false;";
        ViewState[CNT.VS.Table] = "FunctionLoggerCustomer";
    }
    ...
    private void StationChanged(DropDownList ddl)
    {
        DropDownList ddl_FunctionLoggerId = (DropDownList)ddl.FindControl("ddl_FunctionLoggerId");
        ddl_FunctionLoggerId.Items.Clear();
        if (ddl.SelectedValue.IsEmpty()) return;
        U.SetDropDownFunctionLoggerByStation(ddl_FunctionLoggerId, ddl.SelectedValue);
    }
    ...
    private void BindData()
    {
        ...
        List<object> oList = FunctionLoggerCustomer.GetByCriteria(pList, out TotalRow);
    ...
    {
        oList.Add(new FunctionLoggerCustomer() { No = -1, Station = CNT.DataNotAvailable });
    ...
    private void SetInitAdd()
    {
        ...
        List<object> oList = U.GetGridData(gvData, typeof(FunctionLoggerCustomer)).ListData.FindAll(a => ((FunctionLoggerCustomer)a).Station != CNT.DataNotAvailable);
        oList.ForEach(a => { ((FunctionLoggerCustomer)a).IsChecked = false; ((FunctionLoggerCustomer)a).Mode = ""; });
        oList.Insert(0, new FunctionLoggerCustomer() { No = 0, IsChecked = true, Mode = "add" });
        U.BindGrid(gvData, oList);
    ...
    {
        List<object> oList = U.GetGridData(gvData, typeof(FunctionLoggerCustomer)).ListData;
        if (!oList.Exists(a => ((FunctionLoggerCustomer)a).IsChecked))
        {
        ...
        oList.ForEach(a => { ((FunctionLoggerCustomer)a).Mode = ((FunctionLoggerCustomer)a).IsChecked ? "edit" : ""; });
        U.BindGrid(gvData, oList);
        ((CheckBox)gvData.HeaderRow.Cells[0].FindControl("cbCheckAll")).Enabled = false;
    }
    private void Cancel()
    ...
    private void SaveAdd()
    {
        try
        {
            string Result = "";
            List<object> oList = U.GetGridData(gvData, typeof(FunctionLoggerCustomer)).ListData;
            FunctionLoggerCustomer o = (FunctionLoggerCustomer)oList[0];
            ...
            FunctionLoggerCustomer oTemp = FunctionLoggerCustomer.GetByKey(o.StationId, o.FunctionLoggerId, o.CustomerId, o.Id);
            if (oTemp.Id != 0)
            {
                U.ShowMessage($"Save Failed, {Environment.NewLine}Station {o.Station}, Function Logger {o.FunctionLogger}, and Customer {o.Customer} already exist");
                return;
            }
            #endregion
    ...
    private void SaveEdit()
    {
        ...
        foreach (FunctionLoggerCustomer o in oList)
        {
    ...
    private void Delete()
    {
        try
        {
            string ErrorMessage = "";
            List<object> oList = U.GetGridData(gvData, typeof(FunctionLoggerCustomer)).ListData;
            if (oList.FindAll(a => ((FunctionLoggerCustomer)a).IsChecked).Count == 0)
            {
            ...
            {
                FunctionLoggerCustomer o = (FunctionLoggerCustomer)oList[i];
                if (!o.IsChecked) continue;

                ErrorMessage = DataAccess.Delete(0, o.Id, "Id", "FunctionLoggerCustomer");
    ...
    private void ExportExcel()
    {
        ...
        List<object> oList = FunctionLoggerCustomer.GetByCriteria(pList, out double TotalRow);
        ...
        foreach (FunctionLoggerCustomer o in oList)
        {
            ...
            w.SetExcel(o.Station);
            w.SetExcel(o.FunctionLogger);
            w.SetExcel(o.Customer);
            w.Column = 0;
        }
        #endregion
        ...
        resp.Clear();
        resp.Buffer = false;
        resp.AppendHeader("content-disposition", string.Format("attachment; filename=FunctionLoggerCustomer.xlsx"));
        ...
        byte[] buffer = null;
        string tempFilename = $@"{U.PathTempFolder}FunctionLoggerCustomer.xlsx";
