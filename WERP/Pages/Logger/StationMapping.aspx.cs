...
public partial class Pages_Logger_StationMapping : PageBase
{
    #region Handlers
    ...
    protected void gvData_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        else if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            DropDownList ddl_ParentStationId = (DropDownList)e.Row.FindControl("ddl_ParentStationId");
            DropDownList ddl_ChildStationId = (DropDownList)e.Row.FindControl("ddl_ChildStationId");

            List<object> oList = (List<object>)gvData.DataSource;
            StationMapping o = (StationMapping)oList[e.Row.RowIndex];
            cb_IsChecked.Enabled = oList.Exists(a => $"{((StationMapping)a).Mode}" != "") ? false : true;
            U.SetDropDownMasterData(ddl_ParentStationId, "[Station]");
            ddl_ParentStationId.SetValue(o.ParentStationId);
            if (o.ParentStationId.IsNotZero())
            {
                U.SetDropDownChildStation(ddl_ChildStationId, ddl_ChildStationId.SelectedValue);
                ddl_ChildStationId.SetValue(o.ParentStationId);
            }
    ...
    protected void ucPaging_OpChanged(object sender, EventArgs e)
    {
        ...
        List<object> oList = oList = StationMapping.GetByCriteria(pList, out double TotalRow);
    ...
    protected void wSearch_Close(object sender, F.WindowCloseEventArgs e)
    {
        ...
        btnSearch.OnClientClick = wSearch.GetShowReference($"~/Pages/Search.aspx?ClassName=StationMapping&SearchData={arrArgument[1].Replace(Environment.NewLine, "~")}");
    ...
    protected void wSort_Close(object sender, F.WindowCloseEventArgs e)
    {
        ...
        btnSorting.OnClientClick = wSort.GetShowReference($"~/Pages/Sort.aspx?ClassName=StationMapping&SortData={arrArgument[1].Replace(Environment.NewLine, "~")}");
    ...
    protected void ddl_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        switch (ddl.ID)
        {
            case "ddl_ParentStationId":
                ParenStationChanged(ddl);
                break;
        }
    }
    #endregion
    ...
    private void SetInit()
    {
        ...
        btnSearch.OnClientClick = $"{wSearch.GetShowReference($"~/Pages/Search.aspx?ClassName=StationMapping")}return false;";
        btnSorting.OnClientClick = $"{wSort.GetShowReference($"~/Pages/Sort.aspx?ClassName=StationMapping")}return false;";
        ViewState[CNT.VS.Table] = "StationMapping";
    }
    ...
    private void ParenStationChanged(DropDownList ddl)
    {
        DropDownList ddl_ChildStationId = (DropDownList)ddl.FindControl("ddl_ChildStationId");
        ddl_ChildStationId.Items.Clear();
        if (ddl.SelectedValue.IsEmpty()) return;
        U.SetDropDownChildStation(ddl_ChildStationId, ddl.SelectedValue);
    }
    ...
    private void BindData()
    {
        ...
        List<object> oList = StationMapping.GetByCriteria(pList, out TotalRow);
    ...
    {
        oList.Add(new StationMapping() { No = -1, ParentStation = CNT.DataNotAvailable });
    ...
    private void SetInitAdd()
    {
        ...
        List<object> oList = U.GetGridData(gvData, typeof(StationMapping)).ListData.FindAll(a => ((StationMapping)a).ParentStation != CNT.DataNotAvailable);
        oList.ForEach(a => { ((StationMapping)a).IsChecked = false; ((StationMapping)a).Mode = ""; });
        oList.Insert(0, new StationMapping() { No = 0, IsChecked = true, Mode = "add" });
        U.BindGrid(gvData, oList);
    ...
    {
        List<object> oList = U.GetGridData(gvData, typeof(StationMapping)).ListData;
        if (!oList.Exists(a => ((StationMapping)a).IsChecked))
        {
        ...
        oList.ForEach(a => { ((StationMapping)a).Mode = ((StationMapping)a).IsChecked ? "edit" : ""; });
        U.BindGrid(gvData, oList);
        ((CheckBox)gvData.HeaderRow.Cells[0].FindControl("cbCheckAll")).Enabled = false;
    }
    private void Cancel()
    ...
    private void SaveAdd()
    {
        try
        {
            string Result = "";
            List<object> oList = U.GetGridData(gvData, typeof(StationMapping)).ListData;
            StationMapping o = (StationMapping)oList[0];
            ...
            StationMapping oTemp = StationMapping.GetByKey(o.ParentStationId, o.ChildStationId, o.Id);
            if (oTemp.Id != 0)
            {
                U.ShowMessage($"Save Failed, {Environment.NewLine}Parent Station{o.ParentStation} and Child Station {o.ChildStation} already exist");
                return;
            }
            #endregion
    ...
    private void SaveEdit()
    {
        ...
        foreach (StationMapping o in oList)
        {
    ...
    private void Delete()
    {
        try
        {
            string ErrorMessage = "";
            List<object> oList = U.GetGridData(gvData, typeof(StationMapping)).ListData;
            if (oList.FindAll(a => ((StationMapping)a).IsChecked).Count == 0)
            {
            ...
            {
                StationMapping o = (StationMapping)oList[i];
                if (!o.IsChecked) continue;

                ErrorMessage = DataAccess.Delete(0, o.Id, "Id", "StationMapping");
    ...
    private void ExportExcel()
    {
        ...
        List<object> oList = StationMapping.GetByCriteria(pList, out double TotalRow);
        Wrapping w = new Wrapping { Sheet = sheet, CellStyle = HeaderStyle };

        #region HeaderExcel	
        w.SetExcel("No");
        w.SetExcel("Parent Station");
        w.SetExcel("Child Station");
        #endregion
        ...
        foreach (StationMapping o in oList)
        {
            ...
            w.SetExcel(o.ParentStation);
            w.SetExcel(o.ChildStation);
            w.Column = 0;
        }
        #endregion
        ...
        resp.Clear();
        resp.Buffer = false;
        resp.AppendHeader("content-disposition", string.Format("attachment; filename=StationMapping.xlsx"));
        ...
        byte[] buffer = null;
        string tempFilename = $@"{U.PathTempFolder}StationMapping.xlsx";
