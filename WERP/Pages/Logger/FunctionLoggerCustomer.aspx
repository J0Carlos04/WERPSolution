<%@ Page Language="C#" AutoEventWireup="true" CodeFile="FunctionLoggerCustomer.aspx.cs" Inherits="Pages_Logger_FunctionLoggerCustomer" %>
    ...
    <f:Panel ID="pContent" runat="server" ShowBorder="false" ShowHeader="false" Layout="Region" EnableAjaxLoading="true" AjaxLoadingType="Default">
      <Toolbars>
        <f:Toolbar runat="server" ID="tTop" Position="Top"><Items>
          ...
          <f:Button runat="server" ID="btnDelete" Text="Delete" Icon="Delete" OnClick="btn_Click" ConfirmText="Are you sure you want to delete this Function Logger to Customer?. This action cannot be undone. Do you wish to proceed?" ConfirmIcon="Warning" />
            ...
            <asp:GridView runat="server" ID="gvData" AutoGenerateColumns="false" CssClass="table table-striped table-bordered table-hover" OnRowDataBound="gvData_RowDataBound" Width="100%" HeaderStyle-HorizontalAlign="Center">
              <Columns>
                ...
                <asp:TemplateField HeaderText="Station">
                  <ItemTemplate>
                    <asp:Literal runat="server" ID="ltrl_Station" Text='<%# Eval("Station") %>' Visible='<%# Eval("Mode").IsEmpty() %>' />
                    <asp:DropDownList runat="server" ID="ddl_StationId" AutoPostBack="true" OnSelectedIndexChanged="ddl_SelectedIndexChanged" Visible='<%# Eval("Mode").IsNotEmpty() %>' CssClass="form-select form-select-sm" />
                  </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="Function Logger">
                  <ItemTemplate>
                    <asp:Literal runat="server" ID="ltrl_FunctionLogger" Text='<%# Eval("FunctionLogger") %>' Visible='<%# Eval("Mode").IsEmpty() %>' />
                    <asp:DropDownList runat="server" ID="ddl_FunctionLoggerId" Visible='<%# Eval("Mode").IsNotEmpty() %>' CssClass="form-select form-select-sm" />
                  </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="Customer">
                  <ItemTemplate>
                    <asp:Literal runat="server" ID="ltrlCustomer" Text='<%# Eval("Customer") %>' Visible='<%# Eval("Mode").IsEmpty() %>' />
                    <asp:Literal runat="server" ID="ltrl_CustomerId" Text='<%# Eval("CustomerId") %>' Visible="false" />
                    <div style="display:flex;">
                      <asp:TextBox runat="server" ID="tb_Customer" Text='<%# Eval("Customer") %>' Visible='<%# Eval("Mode").IsNotEmpty() %>' CssClass="form-control form-control-sm" ReadOnly="true" />
                      <asp:Button runat="server" ID="btnLookupCust" Text="..." Visible='<%# Eval("Mode").IsNotEmpty() %>' />
                    </div>                    
                  </ItemTemplate>
                </asp:TemplateField>
              </Columns>
