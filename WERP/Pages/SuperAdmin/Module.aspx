<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Module.aspx.cs" Inherits="Pages_SuperAdmin_Module" %>
...
<f:Panel ID="pContent" runat="server" ShowBorder="false" ShowHeader="false" Layout="Region" EnableAjaxLoading="true" AjaxLoadingType="Default">
  <Toolbars>
    <f:Toolbar runat="server" ID="tTop" Position="Top"><Items>
      ...
      <f:Button runat="server" ID="btnDelete" Text="Delete" Icon="Delete" OnClick="btn_Click" ConfirmText="Are you sure you want to delete this Module?. This action cannot be undone. Do you wish to proceed?" ConfirmIcon="Warning" />
        ...
        <asp:GridView runat="server" ID="gvData" AutoGenerateColumns="false" CssClass="table table-striped table-bordered table-hover" OnRowDataBound="gvData_RowDataBound" Width="100%" HeaderStyle-HorizontalAlign="Center">
          <Columns>
            ...
            <asp:TemplateField HeaderText="Page" >              
              <ItemTemplate>		    
                <asp:Literal runat="server" ID="ltrlPage" Text='<%# Eval("Page") %>' Visible='<%# Eval("Mode").IsEmpty() %>' />                        
                <asp:DropDownList runat="server" ID="ddl_Page" Visible='<%# $"{Eval("Mode")}" == "" ? false : true %>' CssClass="form-select form-select-sm" />
              </ItemTemplate>
            </asp:TemplateField>
