<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Role.aspx.cs" Inherits="Pages_SuperAdmin_Role" %>
...
<f:Panel ID="pContent" runat="server" ShowBorder="false" ShowHeader="false" Layout="Region" EnableAjaxLoading="true" AjaxLoadingType="Default">
  <Toolbars>
    <f:Toolbar runat="server" ID="tTop" Position="Top"><Items>
      ...
      <f:Button runat="server" ID="btnDelete" Text="Delete" Icon="Delete" OnClick="btn_Click" ConfirmText="Are you sure you want to delete this Role?. This action cannot be undone. Do you wish to proceed?" ConfirmIcon="Warning" />
        ...
        <asp:GridView runat="server" ID="gvData" AutoGenerateColumns="false" CssClass="table table-striped table-bordered table-hover" OnRowDataBound="gvData_RowDataBound" Width="100%" HeaderStyle-HorizontalAlign="Center">
          <Columns>
            ...
            <asp:TemplateField HeaderText="Name" >              
              <ItemTemplate>		    
                <asp:Literal runat="server" ID="ltrlName" Text='<%# Eval("Name") %>' Visible='<%# $"{Eval("Mode")}" == "" || $"{Eval("Name")}" == "SuperAdmin" || $"{Eval("Name")}" == "Admin" || $"{Eval("Name")}" == "Approver" ? true : false %>' />                        
                <asp:TextBox runat="server" ID="tb_Name" Text='<%# $"{Eval("Mode")}" == "add" ? "" : $"{Eval("Name")}" %>' Visible='<%# $"{Eval("Mode")}" == "" || $"{Eval("Name")}" == "SuperAdmin" || $"{Eval("Name")}" == "Admin" || $"{Eval("Name")}" == "Approver" ? false : true %>' CssClass="form-control form-control-sm" />
              </ItemTemplate>
            </asp:TemplateField>              
