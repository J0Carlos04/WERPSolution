...
public partial class Pages_Inventory_StockOutReturUsed : PageBase
{    
    #region Handlers
    ...
    protected void lb_Click(object sender, EventArgs e)
    {
        ...
        {
            case "lbEdit":
                U.OpenNewTab("EditStockOutReturUsed", "Edit Stock Out Retur", $"getEditWindowUrl('{lb.CommandArgument}', '{lb.CommandName}')");
                break;
        }
    }
    protected void gvData_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        else if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            if (ltrl_Code.Text == CNT.DataNotAvailable) lbEdit.Visible = false;
        }
    }
    protected void ucPaging_OpChanged(object sender, EventArgs e)
    {
        ...
        List<object> oList = oList = StockOutReturUsed.GetByCriteria(pList, out double TotalRow);
    ...
    protected void wSearch_Close(object sender, F.WindowCloseEventArgs e)
    {
        ...
        btnSearch.OnClientClick = wSearch.GetShowReference($"~/Pages/Search.aspx?ClassName=StockOutReturUsed&SearchData={arrArgument[1].Replace(Environment.NewLine, "~")}");
    ...
    protected void wSort_Close(object sender, F.WindowCloseEventArgs e)
    {
        ...
        btnSorting.OnClientClick = wSort.GetShowReference($"~/Pages/Sort.aspx?ClassName=StockOutReturUsed&SortData={arrArgument[1].Replace(Environment.NewLine, "~")}");
        btnSorting.OnClientClick += "return false;";
        BindData();
    }
    protected string GetEditUrl(object id, object StockOutItemId)
    {
        F.JsObjectBuilder joBuilder = new F.JsObjectBuilder();
        joBuilder.AddProperty("id", "EditStockOutReturUsed");
        joBuilder.AddProperty("title", "Edit Stock Out Retur");
        joBuilder.AddProperty("iframeUrl", $"getEditWindowUrl('{id}', '{StockOutItemId}')", true);
    ...
    protected string GetReturUrl(object id)
    {
        F.JsObjectBuilder joBuilder = new F.JsObjectBuilder();
        joBuilder.AddProperty("id", "StockOutReturUsedInput");
        joBuilder.AddProperty("title", "Retur Input");
        joBuilder.AddProperty("iframeUrl", $"getReturWindowUrl('{id}')", true);
        joBuilder.AddProperty("refreshWhenExist", true);
        joBuilder.AddProperty("iconFont", "pencil");
        return String.Format("parent.addExampleTab({0});", joBuilder);
    }
    ...
    private void SetInit()
    {
        ...
        ViewState[CNT.VS.PageName] = "StockOut";
        if (!(U.ViewAccess(ViewState[CNT.VS.PageName]) || U.IsMember(CNT.Approval))) Response.Redirect($"../{CNT.Unauthorized}.aspx");
        btnSearch.OnClientClick = $"{wSearch.GetShowReference($"~/Pages/Search.aspx?ClassName=StockOutReturUsed")}return false;";
        btnSorting.OnClientClick = $"{wSort.GetShowReference($"~/Pages/Sort.aspx?ClassName=StockOutReturUsed")}return false;";
    }
    ...
    {
        F.JsObjectBuilder joBuilder = new F.JsObjectBuilder();
        joBuilder.AddProperty("id", "NewStockOutReturUsed");
        joBuilder.AddProperty("title", "New Stock Out Retur Used");
    ...
    private void BindData()
    {
        ...
        List<object> oList = StockOutReturUsed.GetByCriteria(pList, out TotalRow);
    ...
    {
        oList.Add(new StockOutReturUsed() { No = -1, Code = CNT.DataNotAvailable });
    ...
    private void ExportExcel()
    {
        ...
        List<object> oList = StockOutReturUsed.GetByCriteria(pList, out double TotalRow);
        ...
        w.SetExcel("Code");
        w.SetExcel("Reason");
        w.SetExcel("Item Code");
        w.SetExcel("Item Name");
        w.SetExcel("Retur Qty");
        w.SetExcel("Receiver");
        #endregion
        ...
        foreach (StockOutReturUsed o in oList)
        {
            w.Row += 1;
            w.SetExcel(o.No, true);
            w.SetExcel(o.Code);
            w.SetExcel(o.Reason);
            w.SetExcel(o.ItemCode);
            w.SetExcel(o.ItemName);
            w.SetExcel(o.Qty);
            w.SetExcel(o.Receiver);
            w.Column = 0;
        }
        #endregion
        ...
        resp.Clear();
        resp.Buffer = false;
        resp.AppendHeader("content-disposition", string.Format("attachment; filename=StockOutReturUsed.xlsx"));
        ...
        byte[] buffer = null;
        string tempFilename = $@"{U.PathTempFolder}StockOutReturUsed.xlsx";
