<%@ Page Language="C#" AutoEventWireup="true" CodeFile="StockOutReturUsed.aspx.cs" Inherits="Pages_Inventory_StockOutReturUsed" %>
      ...
      <Content><div class="wrapper-suppress">        
        <asp:GridView runat="server" ID="gvData" AutoGenerateColumns="false" CssClass="table table-striped table-bordered table-hover" OnRowDataBound="gvData_RowDataBound" Width="100%" HeaderStyle-HorizontalAlign="Center">
          <Columns>
            ...
            <asp:TemplateField HeaderText="WorkOrder" ><ItemTemplate><asp:Literal runat="server" ID="ltrl_WorkOrder" Text='<%# Eval("WorkOrderCode") %>' /></ItemTemplate></asp:TemplateField>
...
<f:Window ID="wSort" Title="Sorting" Hidden="true" EnableIFrame="true" runat="server" EnableMaximize="true" EnableResize="true" Target="Top" IsModal="false" Width="562px" Height="470px" OnClose="wSort_Close"></f:Window>                  
  ...
  <script>
        function getNewWindowUrl() {
            return F.baseUrl + 'Pages/Inventory/StockOutReturUsedInput.aspx?parenttabid=' + parent.getActiveTabId();
        }
        function getEditWindowUrl(id, StockOutItemId) {
            return F.baseUrl + 'Pages/Inventory/StockOutReturUsedInput.aspx?Id=' + id + '&StockOutItemId=' + StockOutItemId + '&parenttabid=' + parent.getActiveTabId();
        }        
