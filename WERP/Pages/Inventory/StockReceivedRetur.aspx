<%@ Page Language="C#" AutoEventWireup="true" CodeFile="StockReceivedRetur.aspx.cs" Inherits="Pages_Inventory_StockReceivedRetur" %>
        ...
        <asp:GridView runat="server" ID="gvData" AutoGenerateColumns="false" CssClass="table table-striped table-bordered table-hover" OnRowDataBound="gvData_RowDataBound" Width="100%" HeaderStyle-HorizontalAlign="Center">
          <Columns>
            ...
            <asp:TemplateField HeaderText="Action" ItemStyle-HorizontalAlign="Center"><ItemTemplate>
              <asp:LinkButton runat="server" ID="lbEdit" Text="Edit" CommandArgument='<%# Eval("Id")  %>' CommandName='<%# Eval("StockReceivedItemId")  %>' OnClick="lb_Click"  Font-Underline="false" />              
...
<f:Window ID="wSort" Title="Sorting" Hidden="true" EnableIFrame="true" runat="server" EnableMaximize="true" EnableResize="true" Target="Top" IsModal="false" Width="562px" Height="470px" OnClose="wSort_Close"></f:Window>                  
  ...
  <script>
        function getNewWindowUrl() {
            return F.baseUrl + 'Pages/Inventory/StockReceivedReturInput.aspx?parenttabid=' + parent.getActiveTabId();
        }
        function getEditWindowUrl(id, StockReceivedItemId) {
            return F.baseUrl + 'Pages/Inventory/StockReceivedReturInput.aspx?Id=' + id + '&StockReceivedItemId=' + StockReceivedItemId + '&parenttabid=' + parent.getActiveTabId();
        }        
