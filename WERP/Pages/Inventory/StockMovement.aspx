<%@ Page Language="C#" AutoEventWireup="true" CodeFile="StockMovement.aspx.cs" Inherits="Pages_Inventory_StockMovement" %>
      ...
      <Content><div class="wrapper-suppress">                     
        <asp:GridView runat="server" ID="gvData" AutoGenerateColumns="false" CssClass="table table-striped table-bordered table-hover" Width="100%" HeaderStyle-HorizontalAlign="Center">
          <Columns>            
            ...
            <asp:TemplateField HeaderText="Code" ><ItemTemplate><asp:LinkButton runat="server" ID="lb_Code" Text='<%# Eval("Code") %>' onclick='<%# GetItemUrl(Eval("ItemId")) %>' /></ItemTemplate></asp:TemplateField>
            ...
            <asp:TemplateField HeaderText="Qty" ItemStyle-HorizontalAlign="Right" ><ItemTemplate><asp:Literal runat="server" ID="ltrl_Qty" Text='<%# GetQty(Eval("Qty"), Eval("MovementType")) %>' Visible='<%# $"{Eval("No")}" == "-1" ? false : true %>' /></ItemTemplate></asp:TemplateField>
            <asp:TemplateField HeaderText="Movement Type" ><ItemTemplate><asp:Literal runat="server" ID="ltrl_MovementType" Text='<%# Eval("MovementType") %>' /></ItemTemplate></asp:TemplateField>                                    
            <asp:TemplateField HeaderText="Movement Date" ><ItemTemplate><asp:Literal runat="server" ID="ltrl_MovementDate" Text='<%# $"{Eval("MovementDate")}" == "1/1/0001 12:00:00 AM" ? "" : $"{Eval("MovementDate", "{0: dd-MMM-yyyy}")}" %>' Visible='<%# $"{Eval("No")}" == "-1" ? false : true %>' /></ItemTemplate></asp:TemplateField>
