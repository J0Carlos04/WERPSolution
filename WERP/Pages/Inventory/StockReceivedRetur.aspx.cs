...
public partial class Pages_Inventory_StockReceivedRetur : PageBase
{    
    #region Handlers
    ...
    protected void lb_Click(object sender, EventArgs e)
    {
        ...
        {
            case "lbEdit":
                U.OpenNewTab("EditStockReceivedRetur", "Edit Stock Received Retur", $"getEditWindowUrl('{lb.CommandArgument}', '{lb.CommandName}')");
                break;
        }
    }
    ...
    protected void ucPaging_OpChanged(object sender, EventArgs e)
    {
        ...
        List<object> oList = oList = StockReceivedRetur.GetByCriteria(pList, out double TotalRow);
    ...
    protected void wSearch_Close(object sender, F.WindowCloseEventArgs e)
    {
        ...
        btnSearch.OnClientClick = wSearch.GetShowReference($"~/Pages/Search.aspx?ClassName=StockReceivedRetur&SearchData={arrArgument[1].Replace(Environment.NewLine, "~")}");
    ...
    protected void wSort_Close(object sender, F.WindowCloseEventArgs e)
    {
        ...
        btnSorting.OnClientClick = wSort.GetShowReference($"~/Pages/Sort.aspx?ClassName=StockReceivedRetur&SortData={arrArgument[1].Replace(Environment.NewLine, "~")}");
    ...
    private void SetInit()
    {
        ...
        btnSearch.OnClientClick = $"{wSearch.GetShowReference($"~/Pages/Search.aspx?ClassName=StockReceivedRetur")}return false;";
        btnSorting.OnClientClick = $"{wSort.GetShowReference($"~/Pages/Sort.aspx?ClassName=StockReceivedRetur")}return false;";
    }
    ...
    private void SetInitAdd()
    {
        ...
        joBuilder.AddProperty("id", "NewStockReceivedRetur");
        joBuilder.AddProperty("title", "New Stock Received Retur");
    ...
    private void BindData()
    {
        ...
        List<object> oList = StockReceivedRetur.GetByCriteria(pList, out TotalRow);
    ...
    {
        oList.Add(new StockReceivedRetur() { No = -1, Code = CNT.DataNotAvailable });
    ...
    private void ExportExcel()
    {
        ...
        List<object> oList = StockReceivedRetur.GetByCriteria(pList, out double TotalRow);
        ...
        foreach (StockReceivedRetur o in oList)
        {
            w.Row += 1;
        ...
        resp.Clear();
        resp.Buffer = false;
        resp.AppendHeader("content-disposition", string.Format("attachment; filename=StockReceivedRetur.xlsx"));
        ...
        byte[] buffer = null;        
        string tempFilename = $@"{U.PathTempFolder}StockReceivedRetur.xlsx";
