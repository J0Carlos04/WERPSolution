...
public partial class Pages_Inventory_StockReceived : PageBase
{   
    #region Handlers
    ...
    protected void lb_Click(object sender, EventArgs e)
    {
        ...
        {
            case "lbEdit":
                U.OpenNewTab("EditStockReceived", "Edit Stock Received", $"getEditWindowUrl('{lb.CommandArgument}', '{lb.CommandName}')");
                break;
            case "lbRetur":
                U.OpenNewTab("NewStockReceivedRetur", "New Stock Received Retur", $"getReturWindowUrl('{lb.CommandArgument}')");
                break;
            case "lb_StockOrderCode":
                U.OpenNewTab("ViewStockOrder", "View Stock Order", $"getStockOrderWindowUrl('{lb.CommandArgument}')");
                break;
            case "lb_ItemCode":
                U.OpenNewTab("ViewItem", "View Item", $"getItemWindowUrl('{lb.CommandArgument}')");
                break;
        }
    }
    protected void Download(object sender, EventArgs e)
    {
        ...
        switch (btn.ID)
        {
            case "btnDownloadInvoice":
                DownloadFile(btn, "lb_InvoiceFileName");                
                break;
            case "btnDownloadBast":
                DownloadFile(btn, "lb_BastFileName");
                break;
        }
    }
    protected void gvData_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        {
            Button btnDownloadInvoice = (Button)e.Row.FindControl("btnDownloadInvoice");
            LinkButton lb_InvoiceFileName = (LinkButton)e.Row.FindControl("lb_InvoiceFileName");
            lb_InvoiceFileName.Attributes.Add("onclick", string.Format("ClientChanged('{0}');", btnDownloadInvoice.ClientID));

            Button btnDownloadBast = (Button)e.Row.FindControl("btnDownloadBast");
            LinkButton lb_BastFileName = (LinkButton)e.Row.FindControl("lb_BastFileName");
            lb_BastFileName.Attributes.Add("onclick", string.Format("ClientChanged('{0}');", btnDownloadBast.ClientID));

            LinkButton lbEdit = (LinkButton)e.Row.FindControl("lbEdit");
            lbEdit.Visible = U.UpdateAccess(ViewState[CNT.VS.PageName]);

            Literal ltrl_ItemName = (Literal)e.Row.FindControl("ltrl_ItemName");
            if (ltrl_ItemName.Text == CNT.DataNotAvailable)
            {
                lbEdit.Visible = false;
                LinkButton lbRetur = (LinkButton)e.Row.FindControl("lbRetur");
                lbRetur.Visible = false;
            }
        }
    }
    protected void ucPaging_OpChanged(object sender, EventArgs e)
    {
        ...
        List<object> oList = oList = StockReceivedItem.GetByCriteria(pList, out double TotalRow);
    ...
    protected void wSearch_Close(object sender, F.WindowCloseEventArgs e)
    {
        ...
        btnSearch.OnClientClick = wSearch.GetShowReference($"~/Pages/Search.aspx?ClassName=StockReceivedItem&SearchData={arrArgument[1].Replace(Environment.NewLine, "~")}");
    ...
    protected void wSort_Close(object sender, F.WindowCloseEventArgs e)
    {
        ...
        btnSorting.OnClientClick = wSort.GetShowReference($"~/Pages/Sort.aspx?ClassName=StockReceivedItem&SortData={arrArgument[1].Replace(Environment.NewLine, "~")}");
    ...
    private void SetInit()
    {
        ...
        btnSearch.OnClientClick = $"{wSearch.GetShowReference($"~/Pages/Search.aspx?ClassName=StockReceivedItem")}return false;";
        btnSorting.OnClientClick = $"{wSort.GetShowReference($"~/Pages/Sort.aspx?ClassName=StockReceivedItem")}return false;";        
    }
    ...
    private void SetInitAdd()
    {
        ...
        joBuilder.AddProperty("id", "NewStockReceived");
        joBuilder.AddProperty("title", "New Stock Received");
    ...
    private void DownloadFile(Button btn, string lbFileName)
    {
        ...
        StockReceived o = StockReceived.GetById(lb.CommandArgument);
        byte[] Data = lbFileName.Contains("Invoice") ? o.InvoiceFileData : o.BastFileData;
    ...
    private void BindData()
    {
        ...
        List<object> oList = StockReceivedItem.GetByCriteria(pList, out TotalRow);
    ...
    {
        oList.Add(new StockReceivedItem() { No = -1, ItemName = CNT.DataNotAvailable });
    ...
    private void ExportToExcel()
    {
        ...
        List<object> oList = StockReceivedItem.GetByCriteria(pList, out double TotalRow);
        Wrapping w = new Wrapping { Sheet = sheet, CellStyle = HeaderStyle };

        #region HeaderExcel	
        w.SetExcel("No");
        w.SetExcel("StockReceivedItem No.");
        w.SetExcel("StockReceivedItem Date");
        w.SetExcel("PR No.");
        w.SetExcel("Quot No");
        w.SetExcel("Detail");
        #endregion
        ...
        foreach (StockReceivedItem o in oList)
        {
            ...
            //w.SetExcel(o.StockReceivedItemNo);
            //w.SetExcel(o.FileData);
            //w.SetExcel(o.PRNo);
            //w.SetExcel(o.QuotNo);
            //w.SetExcel(o.Detail);
            w.Column = 0;
        ...
        resp.Clear();
        resp.Buffer = false;
        resp.AppendHeader("content-disStockReceivedItemsition", string.Format("attachment; filename=StockReceived.xlsx"));
        ...
        byte[] buffer = null;
        string tempFilename = $@"{U.PathTempFolder}StockReceived.xlsx";
