...
public partial class Pages_Inventory_StockMovement : PageBase
{
    #region Fields   
    ...
    protected void ucPaging_OpChanged(object sender, EventArgs e)
    {
        ...
        List<object> oList = oList = StockMovement.GetByCriteria(pList, out double TotalRow);
    ...
    protected void wSearch_Close(object sender, F.WindowCloseEventArgs e)
    {
        ...
        btnSearch.OnClientClick = wSearch.GetShowReference($"~/Pages/Search.aspx?ClassName=StockMovement&SearchData={arrArgument[1].Replace(Environment.NewLine, "~")}");
    ...
    protected void wSort_Close(object sender, F.WindowCloseEventArgs e)
    {
        ...
        btnSorting.OnClientClick = wSort.GetShowReference($"~/Pages/Sort.aspx?ClassName=StockMovement&SortData={arrArgument[1].Replace(Environment.NewLine, "~")}");
    ...
    protected object GetQty(object Qty, Object MovementType)
    {
        if ($"{MovementType}" == "StockReceived" || $"{MovementType}" == "StockOutRetur") return Qty;
        else return $"({Qty})";
    }
    ...
    private void SetInit()
    {
        ...
        btnSearch.OnClientClick = $"{wSearch.GetShowReference($"~/Pages/Search.aspx?ClassName=StockMovement")}return false;";
        btnSorting.OnClientClick = $"{wSort.GetShowReference($"~/Pages/Sort.aspx?ClassName=StockMovement")}return false;";
    }
    ...
    private void BindData()
    {
        ...
        List<object> oList = StockMovement.GetByCriteria(pList, out TotalRow);
    ...
    {
        oList.Add(new StockMovement() { No = -1, Name = CNT.DataNotAvailable });
    ...
    private void ExportExcel()
    {
        ...
        List<object> oList = StockMovement.GetByCriteria(pList, out double TotalRow);
        ...
        w.SetExcel("Name");
        w.SetExcel("Qty");
        w.SetExcel("MovementType");
        w.SetExcel("MovementDate");
        ...
        foreach (StockMovement o in oList)
        {
            w.Row += 1;
            ...
            w.SetExcel(o.Name);
            w.SetExcel(o.Qty);
            w.SetExcel(o.MovementType);
            w.SetExcel(o.MovementDate);
        ...
        resp.Clear();
        resp.Buffer = false;
        resp.AppendHeader("content-disposition", string.Format("attachment; filename=StockMovement.xlsx"));
        ...
        byte[] buffer = null;
        string tempFilename = $@"{U.PathTempFolder}StockMovement.xlsx";
