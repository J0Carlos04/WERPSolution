...
public partial class Pages_DataPerumda_PencatatanMeter : PageBase
{    
    #region Handlers
    ...
    protected void ucPaging_OpChanged(object sender, EventArgs e)
    {
        ...
        List<object> oList = oList = Catat.GetByCriteria(pList, out double TotalRow);
    ...
    protected void wSearch_Close(object sender, F.WindowCloseEventArgs e)
    {
        ...
        btnSearch.OnClientClick = wSearch.GetShowReference($"~/Pages/Search.aspx?ClassName=Catat&SearchData={arrArgument[1].Replace(Environment.NewLine, "~")}");
    ...
    protected void wSort_Close(object sender, F.WindowCloseEventArgs e)
    {
        ...
        btnSorting.OnClientClick = wSort.GetShowReference($"~/Pages/Sort.aspx?ClassName=Catat&SortData={arrArgument[1].Replace(Environment.NewLine, "~")}");
    ...
    private void SetInit()
    {
        ...
        btnSearch.OnClientClick = $"{wSearch.GetShowReference($"~/Pages/Search.aspx?ClassName=Catat")}return false;";
        btnSorting.OnClientClick = $"{wSort.GetShowReference($"~/Pages/Sort.aspx?ClassName=Catat")}return false;";
        ViewState[CNT.VS.Table] = "Catat";
    }
    ...
    private void BindData()
    {
        ...
        List<object> oList = Catat.GetByCriteria(pList, out TotalRow);
    ...
    {
        oList.Add(new Catat() { No = -1, thbl = CNT.DataNotAvailable });
    ...
    private void ExportExcel()
    {
        ...
        List<object> oList = Catat.GetByCriteria(pList, out double TotalRow);
        ...
        foreach (Catat o in oList)
        {
            ...
            w.SetExcel(o.ukr);
            w.SetExcel(o.tglcatat);
            w.SetExcel(o.pencatat);
            w.SetExcel(o.tmss);
            w.SetExcel(o.ketcatat);
            w.SetExcel(o.stml);
            w.SetExcel(o.standcatat);
            w.SetExcel(o.stma);
            w.SetExcel(o.taksasi);
            w.SetExcel(o.pakai);
            w.SetExcel(o.lat_long);            
            w.Column = 0;
        }
        #endregion
        ...
        resp.Clear();
        resp.Buffer = false;
        resp.AppendHeader("content-disposition", string.Format("attachment; filename=Catat.xlsx"));
        ...
        byte[] buffer = null;
        string tempFilename = $@"{Server.MapPath("~")}\TempFiles\Catat.xlsx";
