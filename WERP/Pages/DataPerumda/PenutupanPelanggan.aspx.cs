...
public partial class Pages_DataPerumda_PenutupanPelanggan : PageBase
{    
    #region Handlers
    ...
    protected void ucPaging_OpChanged(object sender, EventArgs e)
    {
        ...
        List<object> oList = oList = Tutupan.GetByCriteria(pList, out double TotalRow);
    ...
    protected void wSearch_Close(object sender, F.WindowCloseEventArgs e)
    {
        ...
        btnSearch.OnClientClick = wSearch.GetShowReference($"~/Pages/Search.aspx?ClassName=Tutupan&SearchData={arrArgument[1].Replace(Environment.NewLine, "~")}");
    ...
    protected void wSort_Close(object sender, F.WindowCloseEventArgs e)
    {
        ...
        btnSorting.OnClientClick = wSort.GetShowReference($"~/Pages/Sort.aspx?ClassName=Tutupan&SortData={arrArgument[1].Replace(Environment.NewLine, "~")}");
    ...
    private void SetInit()
    {
        ...
        btnSearch.OnClientClick = $"{wSearch.GetShowReference($"~/Pages/Search.aspx?ClassName=Tutupan")}return false;";
        btnSorting.OnClientClick = $"{wSort.GetShowReference($"~/Pages/Sort.aspx?ClassName=Tutupan")}return false;";
        ViewState[CNT.VS.Table] = "Tutupan";
    }
    ...
    private void BindData()
    {
        ...
        List<object> oList = Tutupan.GetByCriteria(pList, out TotalRow);
    ...
    {
        oList.Add(new Tutupan() { No = -1, noreg = CNT.DataNotAvailable });
    ...
    private void ExportExcel()
    {
        ...
        List<object> oList = Tutupan.GetByCriteria(pList, out double TotalRow);
        Wrapping w = new Wrapping { Sheet = sheet, CellStyle = HeaderStyle };

        #region HeaderExcel	
        w.SetExcel("No");
        w.SetExcel("noreg");
        w.SetExcel("tgl_tutup");
        w.SetExcel("nolg");
        ...
        w.SetExcel("kd_ukmeter");
        w.SetExcel("ukr");
        w.SetExcel("stand_angkat");
        #endregion

        #region DataExcel		
        w.Column = 0;
        w.CellStyle = ItemStyleOdd;
        foreach (Tutupan o in oList)
        {
            w.Row += 1;
            w.SetExcel(o.No, true);
            w.SetExcel(o.noreg);
            w.SetExcel(o.tgl_tutup);
            w.SetExcel(o.nolg);
        ...
        resp.Clear();
        resp.Buffer = false;
        resp.AppendHeader("content-disposition", string.Format("attachment; filename=Tutupan.xlsx"));
        ...
        byte[] buffer = null;
        string tempFilename = $@"{U.PathTempFolder}Tutupan.xlsx";
