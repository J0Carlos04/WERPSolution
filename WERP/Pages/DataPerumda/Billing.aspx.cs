...
public partial class Pages_DataPerumda_Billing : PageBase
{
    #region Fields 
    ...
    protected void ucPaging_OpChanged(object sender, EventArgs e)
    {
        ...
        List<object> oList = oList = Billing.GetByCriteria(pList, out double TotalRow);
    ...
    protected void wSearch_Close(object sender, F.WindowCloseEventArgs e)
    {
        ...
        btnSearch.OnClientClick = wSearch.GetShowReference($"~/Pages/Search.aspx?ClassName=Billing&SearchData={arrArgument[1].Replace(Environment.NewLine, "~")}");
    ...
    protected void wSort_Close(object sender, F.WindowCloseEventArgs e)
    {
        ...
        btnSorting.OnClientClick = wSort.GetShowReference($"~/Pages/Sort.aspx?ClassName=Billing&SortData={arrArgument[1].Replace(Environment.NewLine, "~")}");
    ...
    private void SetInit()
    {
        ...
        btnSearch.OnClientClick = $"{wSearch.GetShowReference($"~/Pages/Search.aspx?ClassName=Billing")}return false;";
        btnSorting.OnClientClick = $"{wSort.GetShowReference($"~/Pages/Sort.aspx?ClassName=Billing")}return false;";
    }
    ...
    private void BindData()
    {
        ...
        List<object> oList = Billing.GetByCriteria(pList, out TotalRow);
    ...
    {
        oList.Add(new Billing() { No = -1, thbl = CNT.DataNotAvailable });
    ...
    private void ExportExcel()
    {
        ...
        List<object> oList = Billing.GetByCriteria(pList, out double TotalRow);
        ...
        w.SetExcel("tglpasangmeter");
        w.SetExcel("tmss");
        w.SetExcel("ketcatat");
        w.SetExcel("stml");
        w.SetExcel("stma");
        w.SetExcel("pakai");
        w.SetExcel("jmlhargaair");
        w.SetExcel("koordinatlat");
        w.SetExcel("koordinatlong");
        #endregion

        #region DataExcel		
        w.Column = 0;
        w.CellStyle = ItemStyleOdd;
        foreach (Billing o in oList)
        {
            w.Row += 1;
            ...
            w.SetExcel(o.almt);
            w.SetExcel(o.trp);
            w.SetExcel(o.namatrp);
            w.SetExcel(o.kd_merkmeter);
            w.SetExcel(o.ukr);
            w.SetExcel(o.ukmeter);
            w.SetExcel(o.nometer);
            w.SetExcel(o.nosegelmeter);
            w.SetExcel(o.tglpasangmeter);
            w.SetExcel(o.tmss);
            w.SetExcel(o.ketcatat);
            w.SetExcel(o.stml);
            w.SetExcel(o.stma);
            w.SetExcel(o.pakai);
            w.SetExcel(o.jmlhargaair);
            w.SetExcel(o.koordinatlat);
            w.SetExcel(o.koordinatlong);
            w.Column = 0;
        }
        #endregion
        ...
        resp.Clear();
        resp.Buffer = false;
        resp.AppendHeader("content-disposition", string.Format("attachment; filename=Billing.xlsx"));
        ...
        byte[] buffer = null;
        string tempFilename = $@"{Server.MapPath("~")}\TempFiles\Billing.xlsx";
