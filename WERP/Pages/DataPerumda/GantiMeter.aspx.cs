...
public partial class Pages_DataPerumda_GantiMeter : PageBase
{    
    #region Handlers
    ...
    protected void ucPaging_OpChanged(object sender, EventArgs e)
    {
        ...
        List<object> oList = oList = GantiMeter.GetByCriteria(pList, out double TotalRow);
    ...
    protected void wSearch_Close(object sender, F.WindowCloseEventArgs e)
    {
        ...
        btnSearch.OnClientClick = wSearch.GetShowReference($"~/Pages/Search.aspx?ClassName=GantiMeter&SearchData={arrArgument[1].Replace(Environment.NewLine, "~")}");
    ...
    protected void wSort_Close(object sender, F.WindowCloseEventArgs e)
    {
        ...
        btnSorting.OnClientClick = wSort.GetShowReference($"~/Pages/Sort.aspx?ClassName=GantiMeter&SortData={arrArgument[1].Replace(Environment.NewLine, "~")}");
    ...
    private void SetInit()
    {
        ...
        btnSearch.OnClientClick = $"{wSearch.GetShowReference($"~/Pages/Search.aspx?ClassName=GantiMeter")}return false;";
        btnSorting.OnClientClick = $"{wSort.GetShowReference($"~/Pages/Sort.aspx?ClassName=GantiMeter")}return false;";
        ViewState[CNT.VS.Table] = "GantiMeter";
    }
    ...
    private void BindData()
    {
        ...
        List<object> oList = GantiMeter.GetByCriteria(pList, out TotalRow);
    ...
    {
        oList.Add(new GantiMeter() { No = -1, no_mohonan_ganmeter = CNT.DataNotAvailable });
    ...
    private void ExportExcel()
    {
        ...
        List<object> oList = GantiMeter.GetByCriteria(pList, out double TotalRow);
        Wrapping w = new Wrapping { Sheet = sheet, CellStyle = HeaderStyle };

        #region HeaderExcel	
        w.SetExcel("No");
        w.SetExcel("no_mohonan_ganmeter");
        w.SetExcel("nolangganan");
        w.SetExcel("tgl_pasang");
        w.SetExcel("nometer_lama");
        w.SetExcel("kd_merek_lama");
        w.SetExcel("ukr_lama");
        w.SetExcel("ukmeter_lama");
        w.SetExcel("nometer");
        w.SetExcel("kd_merekmeter");
        ...
        foreach (GantiMeter o in oList)
        {
            w.Row += 1;
            w.SetExcel(o.No, true);
            w.SetExcel(o.no_mohonan_ganmeter);
            w.SetExcel(o.nolangganan);
            w.SetExcel(o.tgl_pasang);
            w.SetExcel(o.nometer_lama);
            w.SetExcel(o.kd_merek_lama);
            w.SetExcel(o.ukr_lama);
            w.SetExcel(o.ukmeter_lama);
            w.SetExcel(o.nometer);
            w.SetExcel(o.kd_merekmeter);
        ...
        resp.Clear();
        resp.Buffer = false;
        resp.AppendHeader("content-disposition", string.Format("attachment; filename=GantiMeter.xlsx"));
        ...
        byte[] buffer = null;
        string tempFilename = $@"{U.PathTempFolder}GantiMeter.xlsx";
