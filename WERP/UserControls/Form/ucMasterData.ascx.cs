...
public partial class UserControls_Form_ucMasterData : System.Web.UI.UserControl
{
    #region Fields          
    ...
    protected void gvData_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        else if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            Literal ltrlName = (Literal)e.Row.FindControl("ltrlName");
            DropDownList ddl_Active = (DropDownList)e.Row.FindControl("ddl_Active");

            List<object> oList = (List<object>)gvData.DataSource;
            MasterData o = (MasterData)oList[e.Row.RowIndex];
            cb_IsChecked.Enabled = oList.Exists(a => $"{((MasterData)a).Mode}" != "") ? false : true;
            ddl_Active.SelectedValue = $"{o.Active}";

            if (TableName == "RequestSource" && ltrlName.Text == "Social Media")
    ...
    {
        #region GetCiteria
        List<clsParameter> pList = U.GetParameter(ViewState[CNT.VS.Criteria], ViewState[CNT.VS.SortBy], TableName);
        ...
        List<object> oList = oList = MasterData.GetByCriteria(pList, out double TotalRow);
    ...
    protected void wSearch_Close(object sender, F.WindowCloseEventArgs e)
    {
        ...
        btnSearch.OnClientClick = wSearch.GetShowReference($"~/Pages/Search.aspx?ClassName=MasterData&SearchData={arrArgument[1].Replace(Environment.NewLine, "~")}");
    ...
    protected void wSort_Close(object sender, F.WindowCloseEventArgs e)
    {
        ...
        btnSorting.OnClientClick = wSort.GetShowReference($"~/Pages/Sort.aspx?ClassName=MasterData&SortData={arrArgument[1].Replace(Environment.NewLine, "~")}");
    ...
    private void SetInit()
    {
        ...
        btnSearch.OnClientClick = $"{wSearch.GetShowReference($"~/Pages/Search.aspx?ClassName=MasterData")}return false;";
        btnSorting.OnClientClick = $"{wSort.GetShowReference($"~/Pages/Sort.aspx?ClassName=MasterData")}return false;";        
    }    
    ...
    private void BindData()
    {
        ...
        List<object> oList = MasterData.GetByCriteria(pList, out TotalRow);
    ...
    {
        oList.Add(new MasterData() { No = -1, Name = CNT.DataNotAvailable });
    ...
    private void SetInitAdd()
    {
        ...
        List<object> oList = U.GetGridData(gvData, MasterData.MyType).ListData.FindAll(a => ((MasterData)a).Name != CNT.DataNotAvailable);
        oList.ForEach(a => { ((MasterData)a).IsChecked = false; ((MasterData)a).Mode = ""; });
        oList.Insert(0, new MasterData() { No = 0, IsChecked = true, Mode = "add" });
        U.BindGrid(gvData, oList);
    ...
    {
        List<object> oList = U.GetGridData(gvData, MasterData.MyType).ListData;
        if (!oList.Exists(a => ((MasterData)a).IsChecked))
        {
        ...
        oList.ForEach(a => { ((MasterData)a).Mode = ((MasterData)a).IsChecked ? "edit" : ""; });
        U.BindGrid(gvData, oList);
        ((CheckBox)gvData.HeaderRow.Cells[0].FindControl("cbCheckAll")).Enabled = false;        
    }
    private void Cancel()
    ...
    private void SaveAdd()
    {
        try
        {
            string Result = "";
            List<object> oList = U.GetGridData(gvData, MasterData.MyType).ListData;
            MasterData o = (MasterData)oList[0];
            ...
            int Seq = 0;
            if (MasterData.IsFieldExist(TableName, "Name", o.Name, o.Id)) sb.AppendLine($"{Seq.Inc()}. {TableName} with Name : {o.Name} already exist");            
    ...
    private void Delete()
    {
        try
        {
            string Result = "";
            List<object> oList = U.GetGridData(gvData, MasterData.MyType).ListData;
            if (oList.FindAll(a => ((MasterData)a).IsChecked).Count == 0)
            {
            ...
            {
                MasterData o = (MasterData)oList[i];
                if (!o.IsChecked) continue;

                Result = DataAccess.IsDataExist(0, $"{TableName}Id", o.Id);
                if (!Result.IsEmpty())
                {
                ...
                Result = DataAccess.Delete(0, o.Id, "Id", TableName);
                if (Result.Contains("Error Message"))
                {
    ...
    private void ExportExcel()
    {
        ...
        List<object> oList = MasterData.GetByCriteria(pList, out double TotalRow);
        ...
        resp.Clear();
        resp.Buffer = false;
        resp.AppendHeader("content-disposition", $"attachment; filename={TableName}.xlsx");
        ...
        byte[] buffer = null;
        string tempFilename = $@"{U.PathTempFolder}{TableName}.xlsx";
